...
CHAT = Path('lib/presentation/screens/chat/chat_screen.dart')
WIDGET = Path('lib/presentation/widgets/health_contacts_section.dart')

WIDGET_CONTENT = r'''import 'package:cloud_firestore/cloud_firestore.dart';
...
def apply() -> bool:
    ...
    changed = False
    import_line = "import 'package:sehatak/presentation/widgets/health_contacts_section.dart';"
    if import_line not in text:
        anchor = "import 'package:sehatak/presentation/widgets/status_row.dart';"
        text = text.replace(anchor, anchor + '\n' + import_line, 1)
        changed = True
    old = "const labels = ['المحادثات', 'المكالمات', 'جهات الاتصال'];"
    new = "const labels = ['المحادثات', 'المكالمات', 'التواصل الصحي'];"
    if old in text:
        text = text.replace(old, new, 1)
        changed = True
    pattern = re.compile(r"  Widget _buildContactsTab\(bool isDark\) \{.*?\n  \}\n\n  Widget _buildStatusSection", re.S)
    replacement = "  Widget _buildContactsTab(bool isDark) {\n    return HealthContactsSection(isDark: isDark);\n  }\n\n  Widget _buildStatusSection"
    text, count = pattern.subn(replacement, text, count=1)
    if count:
        changed = True
    if changed:
        CHAT.write_text(text, encoding='utf-8')
    WIDGET.write_text(WIDGET_CONTENT, encoding='utf-8')
    return changed


if __name__ == '__main__':
    changed = apply()
    print('Health contact directory applied.' if changed else 'Health contact directory already applied.')
