...
class FileHelper {
  ...
  static Future<String> getChatDirectory(String chatId) async {
    final dir = Directory(path.join(await getAppDirectory(), 'chats', chatId));
  ...
  static Future<bool> fileExists(String filePath) => File(filePath).exists();
  static Future<String> readFile(String filePath) async { final file = File(filePath); if (!await file.exists()) return ''; return file.readAsString(); }
  ...
  static Future<int> getFileSize(String filePath) async { final file = File(filePath); if (!await file.exists()) return 0; return file.length(); }
  ...
  static Future<List<FileSystemEntity>> listFiles(String dirPath) async { final dir = Directory(dirPath); if (!await dir.exists()) return <FileSystemEntity>[]; return dir.list().toList(); }
