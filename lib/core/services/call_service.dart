...
class CallService {
  ...
  Future<T> _retry<T>(Future<T> Function() op) async {
    ...
    for (var i = 0; i < 3; i++) {
      try {
        ...
        last = e;
        if (!['unavailable', 'deadline-exceeded', 'aborted'].contains(e.code) || i == 2) {
          rethrow;
        }
        await Future<void>.delayed(
          Duration(milliseconds: 350 * pow(2, i).toInt()),
        );
      }
    }
  ...
  Stream<List<CallModel>> streamCallHistory({int limit = 50}) => _firestore
      ...
      .snapshots()
      .map((s) => s.docs
          .map((d) => CallModel.fromFirestore(d.id, d.data()))
          .toList());

  Stream<CallModel?> streamCall(String id) => _firestore
      .collection('calls')
      .doc(id)
      .snapshots()
      .map((d) => d.exists ? CallModel.fromFirestore(d.id, d.data()!) : null);
  ...
  Future<CallModel?> initiateCall({
    ...
    if (receiverId.isEmpty || receiverId == uid) {
      throw Exception('معرّف المستقبل غير صالح');
    }
    ...
    await _retry(() => _firestore.runTransaction((tx) async {
      ...
      // The receiver's local ActiveCallRegistry is authoritative on that
      // device. Do not create a local queue here; the receiver coordinator
      // will atomically/defensively mark a competing call as busy.
      tx.set(lockRef, {
    ...
    await _timeline(
      chatId: chatId,
      callId: id,
      text: type == CallType.video ? '📹 بدء مكالمة فيديو' : '📞 بدء مكالمة صوتية',
      status: CallStatus.calling.name,
      type: type,
    );
  ...
  Future<_Ctx?> _state({
    ...
    await _retry(() async {
      await _firestore.runTransaction((tx) async {
        ...
        final currentStatus = CallStatus.values.firstWhere(
          (x) => x.name == raw['status'],
        ...
        final t = CallType.values.firstWhere(
          (x) => x.name == raw['callType'],
          orElse: () => CallType.audio,
        );
        c = _Ctx(raw['chatId']?.toString() ?? '', t);
        tx.update(ref, data);
          ...
          tx.set(
            lockRef,
            {
              ...
              'status': data['status']?.toString() ??
                  (active ? currentStatus.name : CallStatus.ended.name),
              'updatedAt': FieldValue.serverTimestamp(),
            },
  ...
  Future<void> acceptCall(String id) async {
    ...
    // Claim the local slot before the Firestore await. This closes the race
    // where another incoming call arrives between tapping Accept and the
    // connected state being written.
    registry.register(id);
    if (registry.activeCallId != id) {
      await markBusy(id);
      throw StateError('لا يمكن قبول مكالمة ثانية');
    }

    try {
      final c = await _state(
        id: id,
      ...
      if (c != null) {
        await _timeline(
          chatId: c.chatId,
          callId: id,
          text: c.type == CallType.video ? '📹 تم الاتصال بالفيديو' : '📞 تم الاتصال',
          status: CallStatus.connected.name,
          type: c.type,
        );
      }
  ...
  Future<void> markBusy(String id) async {
    final c = await _state(
      id: id,
      allowed: const [CallStatus.calling, CallStatus.ringing],
      data: {
    ...
    if (c != null) {
      await _timeline(
        chatId: c.chatId,
        callId: id,
        text: '📵 المستخدم مشغول بمكالمة أخرى',
        status: CallStatus.busy.name,
        type: c.type,
      );
    }
    ...
    if (c != null) {
      await _timeline(
        ...
        text: durationSeconds != null && durationSeconds > 0
            ? '📞 انتهت المكالمة • ${durationSeconds}s'
            : '📞 انتهت المكالمة',
        status: CallStatus.ended.name,
        type: c.type,
      );
  ...
  Future<void> handleIncomingCall(BuildContext context, RemoteMessage message) async {
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => IncomingCallScreen(
          callId: id,
          callerName: (message.data['callerName'] ?? 'مستخدم').toString(),
          callerId: callerId,
