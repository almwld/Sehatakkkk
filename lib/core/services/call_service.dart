...
class CallService {
  ...
  Future<void> _timeline({required String chatId, required String callId, required String text, required String status, required CallType type}) async {
    ...
    try { await _chat.sendSystemMessage(chatId: chatId, text: text, idempotencyKey: 'call_${callId}_$status', metadata: {'callId': callId, 'callType': type.name, 'status': status}); }
    catch (e) { debugPrint('call timeline: $e'); }
  }

  String _lockId(String a, String b) {
    final ids = [a, b]..sort();
  ...
  Future<CallModel?> initiateCall({required String receiverId, required String receiverName, String? receiverPhotoUrl, required CallType type, required String chatId, String? idempotencyKey}) async {
    ...
    await _retry(() => _firestore.runTransaction((tx) async {
      final existingCall = await tx.get(ref);
      if (existingCall.exists) return;
      ...
      if (lock.exists) {
        ...
        if (activeId != null && activeId.isNotEmpty && ['calling', 'ringing', 'connected'].contains(lockStatus)) {
          final activeRef = _firestore.collection('calls').doc(activeId);
          final active = await tx.get(activeRef);
          if (active.exists && ['calling', 'ringing', 'connected'].contains(active.data()?['status']?.toString())) {
            throw StateError('لديك مكالمة نشطة');
          }
        }
      }
  ...
  Future<_Ctx?> _state({required String id, required List<CallStatus> allowed, required Map<String, dynamic> data, required bool active, bool ignore = false}) async {
    ...
    await _retry(() async {
      await _firestore.runTransaction((tx) async {
        ...
        final currentStatus = CallStatus.values.firstWhere((x) => x.name == raw['status'], orElse: () => CallStatus.calling);
        if (!allowed.contains(currentStatus)) {
          if (ignore) return;
          throw Exception('لا يمكن تغيير حالة المكالمة الحالية');
        }
        final t = CallType.values.firstWhere((x) => x.name == raw['callType'], orElse: () => CallType.audio);
        final rawChatId = raw['chatId'];
        ...
        if (callerId.isNotEmpty && receiverId.isNotEmpty) {
          final lockRef = _firestore.collection('callLocks').doc(_lockId(callerId, receiverId));
          if (active) {
            tx.set(lockRef, {'participants': [callerId, receiverId], 'activeCallId': id, 'status': data['status']?.toString() ?? currentStatus.name, 'updatedAt': FieldValue.serverTimestamp()}, SetOptions(merge: true));
          } else {
            tx.set(lockRef, {'participants': [callerId, receiverId], 'activeCallId': null, 'status': data['status']?.toString() ?? CallStatus.ended.name, 'updatedAt': FieldValue.serverTimestamp()}, SetOptions(merge: true));
          }
        }
      });
      return null;
  ...
  Future<void> rejectCall(String id) async {
    final c = await _state(id: id, allowed: const [CallStatus.calling, CallStatus.ringing], data: {'status': CallStatus.rejected.name, 'endedAt': FieldValue.serverTimestamp()}, active: false);
    await CallSoundCoordinator.instance.stopForCall(id);
  ...
  Future<void> cancelCall(String id) async {
    final c = await _state(id: id, allowed: const [CallStatus.calling, CallStatus.ringing], data: {'status': CallStatus.cancelled.name, 'endedAt': FieldValue.serverTimestamp()}, active: false);
    await CallSoundCoordinator.instance.stopForCall(id);
  ...
  Future<void> endCall(String id, {int? durationSeconds}) async {
    final c = await _state(id: id, allowed: const [CallStatus.calling, CallStatus.ringing, CallStatus.connected], data: {'status': CallStatus.ended.name, 'endedAt': FieldValue.serverTimestamp(), 'durationSeconds': durationSeconds}, active: false);
    await CallSoundCoordinator.instance.stopForCall(id);
  ...
  Future<void> missCall(String id) async {
    final c = await _state(id: id, allowed: const [CallStatus.calling, CallStatus.ringing], data: {'status': CallStatus.missed.name, 'endedAt': FieldValue.serverTimestamp()}, active: false, ignore: true);
    await CallSoundCoordinator.instance.stopForCall(id);
