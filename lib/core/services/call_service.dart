...
class CallService {
    ...
    for (var attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        await Future<void>.delayed(
          Duration(milliseconds: 350 * pow(2, attempt).toInt()),
        );
      }
    }
  ...
  Future<void> _addCallSystemMessage({
    ...
    try {
      ...
      // The call document remains the source of truth. A temporary timeline
      // failure must not tear down an otherwise valid call.
  ...
  Future<CallModel?> initiateCall({
    ...
    if (receiverId.trim().isEmpty || receiverId == userId) {
  ...
  Future<void> acceptCall(String callId) async {
    final result = await _updateCallState(
      callId,
    ...
    await _addCallSystemMessage(
      ...
      callId: callId,
      text: result.type == CallType.video ? '📹 تم الاتصال بالفيديو' : '📞 تم الاتصال',
      status: CallStatus.connected.name,
      type: result.type,
    );
  }
  ...
  Future<_CallContext?> _updateCallState(
    String callId, {
  ...
  Future<String?> resolveChatId(String callId) async {
    final doc = await _withFirestoreRetry(
      () => _firestore.collection('calls').doc(callId).get(),
    );
  ...
  Future<void> handleIncomingCall(BuildContext context, RemoteMessage message) async {
    ...
    if (chatId == null || chatId.trim().isEmpty) {
      chatId = await resolveChatId(callId);
    }
    if (chatId == null || chatId.trim().isEmpty) {
      ToastService.showError('❌ تعذر العثور على المحادثة المرتبطة بالمكالمة');
      return;
    }
    final isVideo = message.data['isVideo'] == 'true' || message.data['isVideoCall'] == 'true';
