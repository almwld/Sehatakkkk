...
class ChatService {
  ...
  Future<DocumentSnapshot<Map<String,dynamic>>> _authorizedChat(String chatId) async {
    final id = _uid(); final snap = await _chatRef(chatId).get();
  ...
  Future<String> createChat({required String doctorId,required String doctorName,required String patientName,String? doctorImage,String? patientImage,String? idempotencyKey}) async {
    final id=_uid(); if(doctorId.isEmpty||doctorId==id) throw Exception('معرّف الطبيب غير صالح');
  ...
  Future<String> sendMessage({required String chatId,required String text,String? imageUrl,String? videoUrl,String? audioUrl,String? fileUrl,String? locationUrl,String? replyToId,String? idempotencyKey,String? fileName,String? fileSize,String? fileMimeType,String? audioDuration}) async {
    ...
    final ref=_chatRef(chatId).collection('messages').doc(); final batch=_firestore.batch();
    ...
    for(final p in participants){if(p!=id)update['unreadCount.$p']=FieldValue.increment(1);} batch.update(_chatRef(chatId),update); await batch.commit(); return ref.id;
  }
  ...
  Future<String> sendSystemMessage({required String chatId,required String text,String? idempotencyKey,Map<String,dynamic>? metadata}) async {
    final id=_uid(); final user=_auth.currentUser!; final chat=await _authorizedChat(chatId);
    if(idempotencyKey?.isNotEmpty==true){final x=await _chatRef(chatId).collection('messages').where('idempotencyKey',isEqualTo:idempotencyKey).limit(1).get();if(x.docs.isNotEmpty)return x.docs.first.id;}
    final participants=List<String>.from(chat.data()?['participants']??const []); final isCall=metadata?['callId']!=null; final type=isCall?'call':'system';
    final ref=_chatRef(chatId).collection('messages').doc(); final batch=_firestore.batch();
    batch.set(ref,{'chatId':chatId,'senderId':id,'senderName':user.displayName??'مستخدم','senderPhotoUrl':user.photoURL,'text':text,'type':type,'metadata':metadata??<String,dynamic>{},'timestamp':FieldValue.serverTimestamp(),'isRead':false,'isDelivered':true,'deliveredAt':FieldValue.serverTimestamp(),'isDeleted':false,'isEdited':false,'reactions':<String,dynamic>{},if(idempotencyKey?.isNotEmpty==true)'idempotencyKey':idempotencyKey});
