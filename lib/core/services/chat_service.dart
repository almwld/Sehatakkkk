...
class ChatService {
  ...
  DocumentReference<Map<String, dynamic>> _chatRef(String chatId) => _firestore.collection('chats').doc(chatId);

  Future<DocumentSnapshot<Map<String, dynamic>>> _authorizedChat(String chatId) async {
    final id = _uid();
    final ref = _chatRef(chatId);
    final snap = await ref.get();
    if (!snap.exists) throw Exception('المحادثة غير موجودة');
    final participants = List<String>.from(snap.data()?['participants'] ?? const []);
    if (!participants.contains(id)) throw Exception('ليس لديك صلاحية لهذه المحادثة');
    return snap;
  }
  ...
  Future<String> createChat({required String doctorId, required String doctorName, required String patientName, String? doctorImage, String? patientImage, String? idempotencyKey}) async {
    ...
    for (final d in existing.docs) {
      ...
      if (p.length == 2 && p.contains(doctorId) && d.data()['isGroup'] != true) return d.id;
    }
  ...
  Future<String> sendMessage({required String chatId, required String text, String? imageUrl, String? videoUrl, String? audioUrl, String? fileUrl, String? locationUrl, String? replyToId, String? idempotencyKey, String? fileName, String? fileSize, String? fileMimeType, String? audioDuration}) async {
    ...
    if (idempotencyKey?.isNotEmpty == true) {
      final x = await _chatRef(chatId).collection('messages').where('idempotencyKey', isEqualTo: idempotencyKey).limit(1).get();
      if (x.docs.isNotEmpty) return x.docs.first.id;
    }
  ...
  Future<String> sendSystemMessage({required String chatId, required String text, String? idempotencyKey, Map<String, dynamic>? metadata}) async {
    final id = await sendMessage(chatId: chatId, text: text, idempotencyKey: idempotencyKey);
    if (metadata != null && metadata.isNotEmpty) await _chatRef(chatId).collection('messages').doc(id).set({'metadata': metadata}, SetOptions(merge: true));
    return id;
  }
  ...
  Future<MessagePaginationResult> getMoreMessages({required String chatId, required int limit, DocumentSnapshot? startAfter}) async {
    ...
    Query<Map<String, dynamic>> q = _chatRef(chatId).collection('messages').orderBy('timestamp', descending: true).limit(limit);
    if (startAfter != null) q = q.startAfterDocument(startAfter);
    final s = await q.get();
  ...
  Future<void> deleteChat(String chatId) async {
    await _authorizedChat(chatId);
    final id = _uid();
    await _chatRef(chatId).update({'deletedFor.$id': true, 'updatedAt': FieldValue.serverTimestamp()});
  }

  Future<void> deleteMessage(String chatId, String messageId) async {
    final id = _uid();
    await _authorizedChat(chatId);
    final ref = _chatRef(chatId).collection('messages').doc(messageId);
    final d = await ref.get();
    if (!d.exists || d.data()?['senderId'] != id) throw Exception('لا يمكن حذف الرسالة');
    await ref.update({'isDeleted': true, 'type': 'deleted', 'text': 'تم حذف هذه الرسالة', 'deletedAt': FieldValue.serverTimestamp()});
  }
  ...
  Future<void> addReaction(String chatId, String messageId, String reaction) async {
    ...
    await _chatRef(chatId).collection('messages').doc(messageId).update({'reactions.$id': reaction.trim()});
  }

  Future<void> markAsRead(String chatId) async {
    final id = _uid();
    await _authorizedChat(chatId);
    final s = await _chatRef(chatId).collection('messages').where('senderId', isNotEqualTo: id).where('isRead', isEqualTo: false).limit(100).get();
    final b = _firestore.batch();
    for (final d in s.docs) b.update(d.reference, {'isRead': true, 'readAt': FieldValue.serverTimestamp()});
    b.update(_chatRef(chatId), {'unreadCount.$id': 0});
    await b.commit();
  }
}
...
class MessagePaginationResult {
  ...
  final bool hasMore;
  const MessagePaginationResult({required this.messages, this.lastDocument, required this.hasMore});
}
