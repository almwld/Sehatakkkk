...
class HomeHealthWidgets extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final vitals = <Map<String, dynamic>>[
      {'label': 'ضغط الدم', 'value': 120.0, 'max': 180.0, 'color': Colors.red, 'icon': 'assets/images/tracking/blood_pressure.png'},
      {'label': 'سكر الدم', 'value': 98.0, 'max': 200.0, 'color': Colors.orange, 'icon': 'assets/images/tracking/blood_sugar.png'},
      {'label': 'اللياقة', 'value': 85.0, 'max': 100.0, 'color': Colors.green, 'icon': 'assets/images/tracking/fitness.png'},
      {'label': 'الوزن', 'value': 72.0, 'max': 120.0, 'color': Colors.purple, 'icon': 'assets/images/tracking/weight_tracking.png'},
      {'label': 'التغذية', 'value': 8.0, 'max': 10.0, 'color': Colors.teal, 'icon': 'assets/images/tracking/nutrition.png'},
      {'label': 'الصحة النفسية', 'value': 9.0, 'max': 10.0, 'color': Colors.indigo, 'icon': 'assets/images/tracking/mental_health.png'},
    ];
    ...
    return Column(children: [
      ...
      Padding(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: isDark ? const Color(0xFF1A2540) : Colors.white, borderRadius: BorderRadius.circular(22)),
          child: Row(children: [
      ...
      SizedBox(height: 154, child: ListView.separated(padding: const EdgeInsets.symmetric(horizontal: 16), scrollDirection: Axis.horizontal, itemCount: vitals.length, separatorBuilder: (_, __) => const SizedBox(width: 10), itemBuilder: (_, i) => _vital(vitals[i]))),
    ]);
  }

  Widget _header(String title) => Padding(padding: const EdgeInsets.fromLTRB(16, 18, 16, 8), child: Row(children: [Expanded(child: Text(title, style: TextStyle(fontSize: 17, fontWeight: FontWeight.w900, color: isDark ? Colors.white : const Color(0xFF212121)))), TextButton(onPressed: () => onNavigate(AppRouter.dashboard), child: const Text('عرض الكل', style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.w700, fontSize: 13)))]));
...
class _VitalPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(covariant _VitalPainter old) => old.progress != progress || old.color != color;
}
