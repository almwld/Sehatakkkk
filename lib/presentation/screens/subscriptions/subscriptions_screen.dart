...
class _SubscriptionsScreenState extends State<SubscriptionsScreen> {
  ...
  final List<_Plan> _plans = const [
    _Plan('الباقة المجانية', 'free', 0, 0, Icons.volunteer_activism_rounded, 'الأساسيات الصحية اليومية بدون رسوم', ['3 استشارات مجانية شهرياً', 'سجل صحي إلكتروني', 'تذكير بالمواعيد', 'تصفح الأدوية والأسعار']),
    _Plan('الباقة الفضية', 'silver', 3000, 30000, Icons.workspace_premium_rounded, 'رعاية صحية منتظمة بسعر مناسب', ['10 استشارات شهرياً', 'خصم 20% على الأدوية', 'تحليل منزلي مجاني شهرياً', 'متابعة دورية مع طبيب', 'تقارير صحية شهرية']),
    _Plan('الباقة البرونزية', 'bronze', 3900, 39000, Icons.shield_rounded, 'مزايا متقدمة لرعاية صحية أكثر شمولاً', ['20 استشارة شهرياً', 'خصم 25% على الأدوية', 'تحليل منزلي مجاني شهرياً', 'أولوية متوسطة في الحجز', 'تقارير صحية شهرية', 'متابعة صحية أساسية']),
    _Plan('الباقة الذهبية', 'gold', 4900, 35000, Icons.auto_awesome_rounded, 'أفضل قيمة للرعاية الصحية المتكاملة', ['استشارات غير محدودة 24/7', 'خصم 35% على الأدوية', 'تحاليل منزلية مجانية', 'أولوية في الحجز', 'طبيب شخصي مخصص', 'تقارير صحية أسبوعية', 'دعم فني VIP'], popular: true),
    _Plan('باقة العائلة', 'family', 7500, 75000, Icons.family_restroom_rounded, 'رعاية متكاملة لك ولعائلتك حتى 5 أفراد', ['كل مميزات الباقة الذهبية', 'حتى 5 أفراد من العائلة', 'استشارات أطفال مجانية', 'متابعة الحمل والولادة', 'طبيب عائلة مخصص', 'خصم 50% على الأدوية']),
    _Plan('الباقة الكريستالية', 'crystal', 12000, 120000, Icons.diamond_rounded, 'تجربة رعاية صحية فائقة ومتكاملة', ['كل مميزات باقة العائلة', 'حتى 8 أفراد من العائلة', 'استشارات غير محدودة 24/7', 'طبيب شخصي وكبير أطباء مخصص', 'أولوية قصوى في الحجوزات', 'تحاليل منزلية متقدمة', 'خصم 60% على الأدوية', 'مدير رعاية صحية شخصي', 'دعم VIP على مدار الساعة', 'تقارير صحية متقدمة']),
  ];
  ...
  void _subscribe(_Plan plan) {
    ...
    Navigator.push<bool>(context, MaterialPageRoute(builder: (_) => SubscriptionPaymentScreen(planName: plan.name, planCode: plan.code, price: price, annual: _annualBilling, planEmoji: _emoji(plan.icon)))).then((success) {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(child: ListView(padding: const EdgeInsets.all(14), children: [_header(), const SizedBox(height: 14), _billingToggle(), const SizedBox(height: 14), ...List.generate(_plans.length, (i) => Padding(padding: const EdgeInsets.only(bottom: 12), child: _card(_plans[i], i))), _trustNote()])),
  );
  ...
  Widget _card(_Plan plan, int index) {
    ...
    return Container(decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: selected || plan.popular ? AppColors.primary : const Color(0xFFE1E6E8), width: selected || plan.popular ? 1.5 : 1)), child: Column(children: [
      ...
      Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        const Divider(height: 24), ...plan.features.map(_feature), const SizedBox(height: 8),
  ...
  Widget _trustNote() => Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14)), child: const Row(children: [Icon(Icons.lock_outline_rounded, color: AppColors.primary), SizedBox(width: 8), Expanded(child: Text('الدفع من محفظة صحتك، مع فاتورة إلكترونية مرتبطة بمعاملة الدفع والاشتراك.', style: TextStyle(fontSize: 11, color: AppColors.grey)))]));
}
...
class _Plan {
  ...
  final bool popular;
  const _Plan(this.name, this.code, this.monthly, this.annual, this.icon, this.description, this.features, {this.popular = false});
}
