...
class _SubscriptionsScreenState extends State<SubscriptionsScreen>{
  ...
  Widget _card((String,String,int,int,String,List<String>) p,bool dark){
    final price=_annual?p.$4:p.$3;final featured=p.$2=='gold'||p.$2=='family';
    return Container(margin:const EdgeInsets.only(bottom:12),padding:const EdgeInsets.all(18),decoration:BoxDecoration(color:dark?const Color(0xFF122027):Colors.white,borderRadius:BorderRadius.circular(22),border:Border.all(color:featured?AppColors.primary.withOpacity(.45):Colors.transparent,width:1.2),boxShadow:[BoxShadow(color:Colors.black.withOpacity(.04),blurRadius:14,offset:const Offset(0,5))]),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      Row(children:[Container(width:46,height:46,decoration:BoxDecoration(color:AppColors.primary.withOpacity(.1),borderRadius:BorderRadius.circular(14)),child:Image.asset('assets/images/services/packages.png',width:30,height:30,errorBuilder:(_,__,___)=>const Icon(Icons.workspace_premium_outlined,color:AppColors.primary))),const SizedBox(width:12),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(p.$1,style:const TextStyle(fontSize:18,fontWeight:FontWeight.w900)),Text(p.$5,style:TextStyle(fontSize:11,color:dark?Colors.white60:Colors.black54))])),if(featured)Container(padding:const EdgeInsets.symmetric(horizontal:8,vertical:4),decoration:BoxDecoration(color:AppColors.primary.withOpacity(.1),borderRadius:BorderRadius.circular(10)),child:const Text('مميزة',style:TextStyle(color:AppColors.primary,fontSize:10,fontWeight:FontWeight.w800)))]),
      const SizedBox(height:14),Text(price==0?'مجاناً':'\${_money(price)} ر.ي',style:const TextStyle(fontSize:27,fontWeight:FontWeight.w900,color:AppColors.primary)),if(price>0)Text(_annual?'للسنة كاملة':'لكل شهر',style:TextStyle(fontSize:11,color:dark?Colors.white54:Colors.black45)),
      const SizedBox(height:10),...p.$6.map((f)=>Padding(padding:const EdgeInsets.only(bottom:5),child:Row(children:[const Icon(Icons.check_circle_outline,color:AppColors.primary,size:17),const SizedBox(width:7),Expanded(child:Text(f,style:const TextStyle(fontSize:12)))]))),
