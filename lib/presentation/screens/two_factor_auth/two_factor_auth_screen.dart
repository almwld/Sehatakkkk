...
class _TwoFactorAuthScreenState extends State<TwoFactorAuthScreen> {
  ...
  void _verifySetup() { setState(() => _verificationStep = 2); }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(
            ...
            decoration: BoxDecoration(gradient: LinearGradient(colors: _isEnabled ? [AppColors.success, AppColors.teal] : [AppColors.grey, AppColors.darkGrey]), borderRadius: BorderRadius.circular(16)),
          ...
          if (_verificationStep == 1) ...[
            Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Center(child: Container(width: 180, height: 180, decoration: BoxDecoration(color: AppColors.primary.withOpacity(0.05), borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.primary.withOpacity(0.2))), child: const Center(child: Icon(Icons.qr_code_2, size: 120, color: AppColors.primary)))),
                const SizedBox(height: 12),
                _codeBox('المفتاح السري:', _secretKey),
                const SizedBox(height: 16),
                const Text('2. أدخل رمز التحقق', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                const SizedBox(height: 8),
                Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: List.generate(6, (i) => SizedBox(width: 45, height: 55, child: TextField(controller: _digitControllers[i], focusNode: _focusNodes[i], textAlign: TextAlign.center, keyboardType: TextInputType.number, maxLength: 1, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold), decoration: InputDecoration(counterText: '', border: OutlineInputBorder(borderRadius: BorderRadius.circular(8))), onChanged: (v) { if (v.isNotEmpty && i < 5) _focusNodes[i + 1].requestFocus(); }))),
                const SizedBox(height: 16),
              ...
              child: Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  if (_showBackupCodes) ...[
                    ...
                    const SizedBox(height: 8),
                    ..._backupCodes.map((c) => Container(margin: const EdgeInsets.only(bottom: 4), padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: AppColors.surfaceContainerLow, borderRadius: BorderRadius.circular(8)), child: Row(children: [const Icon(Icons.content_copy, size: 14, color: AppColors.grey), const SizedBox(width: 8), Text(c, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14, letterSpacing: 2))]))),
                  ],
                ]),
              ),
            ),
  ...
  Widget _infoCard(IconData icon, String title, String desc, Color color) => Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: color.withOpacity(0.05), borderRadius: BorderRadius.circular(14), border: Border.all(color: color.withOpacity(0.2))), child: Row(children: [Icon(icon, color: color, size: 28), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: color)), Text(desc, style: const TextStyle(fontSize: 11, color: AppColors.darkGrey))]))]));

  Widget _methodSelector() => Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 6)]), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget _codeBox(String label, String code) => Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: AppColors.surfaceContainerLow, borderRadius: BorderRadius.circular(10)), child: Row(children: [Text(label, style: const TextStyle(fontSize: 12, color: AppColors.grey)), const Spacer(), Text(code, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18, letterSpacing: 2, color: AppColors.primary)), const SizedBox(width: 8), GestureDetector(onTap: () {}, child: const Icon(Icons.content_copy, size: 18, color: AppColors.grey))]));
}
