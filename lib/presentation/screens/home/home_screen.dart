...
class _HomeScreenState extends State<HomeScreen> {
  ...
  BottomNavigationBarItem _buildNavItem(String iconName, String label, int index, {bool isSpecial = false}) {
    ...
    // ✅ استخدام المسار الصحيح للمجلد core
...
class _HomeTabState extends State<HomeTab> {
  ...
  // ✅ مسارات البنرات الصحيحة
  final List<Map<String, dynamic>> _banners = [
    {'title': 'رعاية صحية متميزة', 'sub': 'احجز موعدك الآن مع أفضل الأطباء', 'image': 'assets/images/banners/banner_1.png'},
    {'title': 'صيدليتك في راحة يدك', 'sub': 'اطلب أدويتك وتصلك في أسرع وقت', 'image': 'assets/images/banners/banner_2.png'},
    {'title': 'مختبرات متطورة', 'sub': 'نتائج دقيقة وسريعة', 'image': 'assets/images/banners/banner_3.png'},
  ];
  ...
  final List<Map<String, dynamic>> _communityPosts = [
    {'id': 1, 'author': 'د. سارة العمري', 'avatar': ImageService.doctor2, 'image': 'assets/images/banners/banner_1.png', 'title': 'نصائح للعناية بالبشرة', 'content': 'مع حلول فصل الصيف، احرصي على ترطيب بشرتك واستخدام واقي الشمس.', 'likes': 120, 'comments': ['تعليق 1', 'تعليق 2'], 'shares': 15, 'time': 'منذ ساعة', 'liked': false, 'bookmarked': false},
    {'id': 2, 'author': 'د. خالد النخلاني', 'avatar': ImageService.doctor2, 'image': 'assets/images/banners/banner_2.png', 'title': 'أهمية الفيتامينات', 'content': 'الفيتامينات عناصر أساسية لصحة الجسم، تأكد من تناولها.', 'likes': 95, 'comments': ['تعليق 1'], 'shares': 8, 'time': 'منذ 3 ساعات', 'liked': false, 'bookmarked': false},
    {'id': 3, 'author': 'د. أحمد المولد', 'avatar': ImageService.doctor1, 'image': 'assets/images/banners/banner_3.png', 'title': 'صحة القلب', 'content': 'القلب محرك الحياة، احرص على الرياضة والأكل الصحي.', 'likes': 210, 'comments': ['تعليق 1', 'تعليق 2', 'تعليق 3'], 'shares': 22, 'time': 'منذ 5 ساعات', 'liked': true, 'bookmarked': true},
  ];
  ...
  void _sharePost(int index) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('تم مشاركة: ${post['title']}'), backgroundColor: Colors.green, duration: const Duration(seconds: 2)),
    );
  }
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text('التعليقات (${post['comments'].length})', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context)),
              ],
            ),
            const Divider(),
            if (post['comments'].isEmpty)
              const Center(child: Padding(padding: EdgeInsets.all(20), child: Text('لا توجد تعليقات', style: TextStyle(color: Colors.grey)))),
            SizedBox(height: 16),
            Row(
              children: [
                Expanded(
                ...
                children: [
                  ClipRRect(
                    ...
                    child: CachedNetworkImage(
                      ...
                      errorWidget: (_, __, ___) => Container(width: 55, height: 55, color: Colors.grey[200], child: const Icon(Icons.person, color: Colors.grey)),
                    ),
                  ),
                  const SizedBox(width: 10),
                  Expanded(
                    child: Column(
                      ...
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Text(doctor['name'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13), maxLines: 1, overflow: TextOverflow.ellipsis),
                        Text(doctor['specialty'], style: const TextStyle(fontSize: 11, color: Colors.grey), maxLines: 1, overflow: TextOverflow.ellipsis),
                        Row(
                          children: [
                            ...
                            Text('(${doctor['reviews']})', style: const TextStyle(fontSize: 10, color: Colors.grey)),
                            const Spacer(),
                            Container(width: 8, height: 8, decoration: BoxDecoration(color: doctor['available'] ? Colors.green : Colors.red, shape: BoxShape.circle)),
                          ],
                        ),
                      ],
                    ),
              ...
              children: [
                ClipRRect(
                  ...
                  child: CachedNetworkImage(
                    ...
                    errorWidget: (_, __, ___) => Container(height: 50, width: 50, color: Colors.grey[200], child: const Icon(Icons.medication, color: Colors.grey)),
                  ),
                ),
                ...
                Container(
                  ...
                  child: Text(product['category'], style: const TextStyle(fontSize: 8, color: Colors.grey)),
                ),
                const Spacer(),
                Text('${product['price']} ر.ي', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Color(0xFF0D5257))),
              ],
            ),
          );
        },
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(tip['subtitle'] as String, style: const TextStyle(fontSize: 10, color: Colors.grey), textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis),
            ],
          ),
        );
      },
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Row(
              children: [
                CircleAvatar(radius: 20, backgroundImage: CachedNetworkImageProvider(post['avatar']), child: const Icon(Icons.person, size: 20)),
                const SizedBox(width: 10),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(post['author'], style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: isDark ? Colors.white : Colors.black87)),
                      Text(post['time'], style: TextStyle(fontSize: 11, color: isDark ? Colors.grey[400] : Colors.grey[600])),
                    ],
                  ),
                ),
                IconButton(icon: Icon(Icons.more_horiz, color: isDark ? Colors.grey[400] : Colors.grey[600]), onPressed: () {}),
              ],
            ),
          ),
          ClipRRect(
            child: CachedNetworkImage(
              ...
              fit: BoxFit.cover,
              placeholder: (context, url) => Container(height: 200, color: isDark ? Colors.grey[800] : Colors.grey[200], child: const Center(child: CircularProgressIndicator(strokeWidth: 2))),
              errorWidget: (context, url, error) => Container(height: 200, color: isDark ? Colors.grey[800] : Colors.grey[200], child: const Icon(Icons.image, color: Colors.grey, size: 40)),
            ),
          ),
          Padding(
            ...
            child: Row(
              children: [
                GestureDetector(
                  ...
                  child: Icon(post['liked'] ? Icons.favorite : Icons.favorite_border, color: post['liked'] ? Colors.red : (isDark ? Colors.grey[400] : Colors.grey[600]), size: 26),
                ),
                const SizedBox(width: 4),
                ...
                GestureDetector(
                  ...
                  child: Icon(Icons.chat_bubble_outline, color: isDark ? Colors.grey[400] : Colors.grey[600], size: 24),
                ),
                const SizedBox(width: 4),
                Text('${post['comments'].length}', style: TextStyle(fontSize: 12, color: isDark ? Colors.grey[400] : Colors.grey[600])),
                const SizedBox(width: 16),
                GestureDetector(
                  onTap: () => _sharePost(index),
                  child: Icon(Icons.repeat, color: isDark ? Colors.grey[400] : Colors.grey[600], size: 24),
                ),
                const SizedBox(width: 4),
                ...
                GestureDetector(
                  ...
                  child: Icon(post['bookmarked'] ? Icons.bookmark : Icons.bookmark_border, color: post['bookmarked'] ? AppColors.primary : (isDark ? Colors.grey[400] : Colors.grey[600]), size: 24),
                ),
              ],
            ),
          ),
          Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(post['title'], style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: isDark ? Colors.white : Colors.black87)),
                const SizedBox(height: 2),
                Text(post['content'], style: TextStyle(fontSize: 13, color: isDark ? Colors.grey[300] : Colors.grey[700]), maxLines: 2, overflow: TextOverflow.ellipsis),
              ],
            ),
          ),
          Padding(
