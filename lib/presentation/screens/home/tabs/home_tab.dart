...
class CircularProgressPainter extends CustomPainter {
  ...
  CircularProgressPainter({required this.progress, required this.backgroundColor, required this.progressColor, this.strokeWidth = 5});
  @override
  void paint(Canvas canvas, Size size) {
    final center = Offset(size.width / 2, size.height / 2);
    final radius = size.width / 2 - strokeWidth / 2;
    final backgroundPaint = Paint()..color = backgroundColor..strokeWidth = strokeWidth..style = PaintingStyle.stroke..strokeCap = StrokeCap.round;
...
// ============================================================
// 🏠 HomeTab - الشاشة الرئيسية المتكاملة
// ============================================================
...
class _HomeTabState extends State<HomeTab> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> _fetchDataFromFirebase() async {
    try {
      ...
      final doctorsSnapshot = await FirebaseFirestore.instance.collection('doctors').limit(10).get();
      if (doctorsSnapshot.docs.isNotEmpty && mounted) {
        final List<Map<String, dynamic>> firebaseDoctors = [];
        for (var doc in doctorsSnapshot.docs) {
          final data = doc.data();
          firebaseDoctors.add({'id': doc.id, 'name': data['name'] ?? 'دكتور', 'specialty': data['specialty'] ?? 'طبيب عام', 'rating': data['rating'] ?? 4.0, 'reviews': data['reviews'] ?? 0, 'image': data['image'] ?? ImageKit.doctor1});
        }
        setState(() { _topDoctors = [..._defaultTopDoctors, ...firebaseDoctors]; });
        print('✅ تم جلب ${firebaseDoctors.length} طبيب جديد');
      }
      final productsSnapshot = await FirebaseFirestore.instance.collection('products').limit(10).get();
      if (productsSnapshot.docs.isNotEmpty && mounted) {
        final List<Map<String, dynamic>> firebaseProducts = [];
        for (var doc in productsSnapshot.docs) {
          final data = doc.data();
          firebaseProducts.add({'name': data['name'] ?? 'منتج', 'price': data['price'] ?? 0, 'image': data['image'] ?? ImageKit.medicine1, 'category': data['category'] ?? 'عام', 'discount': data['discount'] ?? 0, 'rating': data['rating'] ?? 4.0});
        }
        setState(() { _products = [..._defaultProducts, ...firebaseProducts]; });
        print('✅ تم جلب ${firebaseProducts.length} منتج جديد');
      }
    } catch (e) { print('⚠️ Error fetching data from Firebase: $e'); }
  }
  ...
  void _sharePost(int index) {
    ...
    Share.share('📢 ${post['title']}\n\n${post['content']}\n\n👁️ ${post['views']} مشاهدة\n❤️ ${post['likes']} إعجاب\n💬 ${post['comments']} تعليق\n\n---\nمنشور من تطبيق صحتك - Sehatak', subject: post['title']);
  }
  ...
  void _toggleLike(int index) {
    setState(() {
      ...
      if (isLiked) { _communityPosts[index]['likes']--; } else { _communityPosts[index]['likes']++; }
    });
  }

  void _savePost(int index) {
    setState(() { _communityPosts[index]['saved'] = !(_communityPosts[index]['saved'] ?? false); });
  ...
  void _showComments(Map<String, dynamic> post, int index) {
    showModalBottomSheet(context: context, isScrollControlled: true, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) => _buildCommentsSheet(post, index));
  }
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Row(children: [Expanded(child: Text('التعليقات (${post['comments']})', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold))), IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))]),
          const Divider(),
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              stream: FirebaseFirestore.instance.collection('community_posts').doc(post['id'].toString()).collection('comments').orderBy('timestamp', descending: true).snapshots(),
              builder: (context, snapshot) {
                if (snapshot.hasError) return Center(child: Text('حدث خطأ: ${snapshot.error}'));
                if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
                final comments = snapshot.data?.docs ?? [];
                if (comments.isEmpty) return const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.chat_bubble_outline, size: 60, color: Colors.grey), SizedBox(height: 16), Text('لا توجد تعليقات، كن أول من يعلق!')]));
                return ListView.builder(itemCount: comments.length, itemBuilder: (context, i) { final data = comments[i].data() as Map<String, dynamic>; return _buildCommentTile(data, post['id'].toString(), index); });
              },
            ),
          ),
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      suffixIcon: IconButton(icon: const Icon(Icons.tag, size: 18, color: Colors.grey), onPressed: () { ToastService.showSuccess('اكتب # لبدء الوسم'); }),
                    ),
                    textDirection: TextDirection.rtl,
                    onChanged: (value) { if (value.endsWith('@')) ToastService.showSuccess('اقتراحات المستخدمين...'); },
                  ),
                ),
                const SizedBox(width: 8),
                CircleAvatar(backgroundColor: AppColors.primary, child: IconButton(icon: const Icon(Icons.send, color: Colors.white, size: 20), onPressed: () { final comment = _commentController.text.trim(); if (comment.isNotEmpty) { _addComment(index, comment); _commentController.clear(); _focusNode.unfocus(); } })),
              ],
            ),
        ],
      ),
  ...
  Widget _buildCommentTile(Map<String, dynamic> data, String postId, int postIndex) {
    ...
    return Column(
      ...
      children: [
        ListTile(
          leading: CircleAvatar(backgroundColor: AppColors.primary.withOpacity(0.1), child: Text((data['userName'] ?? 'م')[0], style: TextStyle(color: AppColors.primary))),
          title: GestureDetector(onTap: () { ToastService.showSuccess('@${data['userName']}'); }, child: Text(data['userName'] ?? 'مستخدم')),
          subtitle: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(data['comment'] ?? ''),
              if (data['comment'] != null && data['comment'].contains('#'))
                Wrap(children: (data['comment'] as String).split(' ').where((word) => word.startsWith('#')).map((tag) => GestureDetector(onTap: () { ToastService.showSuccess('البحث عن الوسم: $tag'); }, child: Text(tag, style: TextStyle(color: AppColors.primary)))).toList()),
            ],
          ),
          trailing: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              IconButton(icon: const Icon(Icons.reply, size: 16), onPressed: () { ToastService.showSuccess('رد على التعليق قيد التطوير'); }),
              IconButton(icon: const Icon(Icons.flag, size: 16, color: Colors.grey), onPressed: () { ToastService.showSuccess('تم الإبلاغ عن التعليق'); }),
              Text(_formatTime((data['timestamp'] as Timestamp?)?.toDate()), style: TextStyle(fontSize: 10, color: Colors.grey[500])),
            ],
          ),
        ),
        if (hasReplies)
          Padding(
            ...
            child: Column(children: (data['replies'] as List).map((reply) {
              return ListTile(
                leading: CircleAvatar(radius: 14, backgroundColor: AppColors.primary.withOpacity(0.1), child: Text((reply['userName'] ?? 'م')[0], style: TextStyle(color: AppColors.primary, fontSize: 10))),
  ...
  Future<void> _addComment(int index, String comment) async {
    ...
    try {
      await FirebaseFirestore.instance.collection('community_posts').doc(postId).collection('comments').add({'userId': user.uid, 'userName': user.displayName ?? 'مستخدم', 'userAvatar': user.photoURL ?? '', 'comment': comment, 'timestamp': FieldValue.serverTimestamp(), 'replies': [], 'mentions': _extractMentions(comment), 'hashtags': _extractHashtags(comment)});
      await FirebaseFirestore.instance.collection('community_posts').doc(postId).update({ 'comments': FieldValue.increment(1) });
      if (!mounted) return;
      setState(() { _communityPosts[index]['comments']++; });
      ToastService.showSuccess('✅ تم إضافة التعليق');
      final mentions = _extractMentions(comment);
      for (var mentionedUser in mentions) { await _notificationService.sendMentionNotification(mentionedUser, user.displayName ?? 'مستخدم'); }
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            Center(child: Container(width: 40, height: 4, decoration: BoxDecoration(color: Colors.grey[400], borderRadius: BorderRadius.circular(2)))),
            const SizedBox(height: 16),
            Row(children: [Image.asset(tip['icon'] as String, width: 48, height: 48, errorBuilder: (_, __, ___) => Icon(Icons.image, color: Colors.grey[600], size: 40)), const SizedBox(width: 12), Expanded(child: Text(tip['title'] as String, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)))]),
            const SizedBox(height: 8),
            ...
            SizedBox(width: double.infinity, child: ElevatedButton(onPressed: () => Navigator.pop(context), style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), child: const Text('إغلاق'))),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading && !_dataLoaded) return _buildShimmerLoader(isDark);
    if (_hasError && !_dataLoaded) return _buildErrorScreen(isDark);
          ...
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                ...
                const SizedBox(height: 16),
                if (_isLoggedIn) ...[_buildStatsRow(), const SizedBox(height: 16)],
                ...
                _buildSectionWithCustomization('اكتشف المزيد', isDark, _buildDiscoverGrid(isDark)),
                if (_weatherData != null) _buildWeatherWidget(),
                if (_aiRecommendation != null) _buildAIRecommendation(),
  ...
  Widget _buildShimmerLoader(bool isDark) {
    return Shimmer.fromColors(baseColor: isDark ? Colors.grey[800]! : Colors.grey[300]!, highlightColor: isDark ? Colors.grey[700]! : Colors.grey[100]!,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(_errorMessage, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w500, color: isDark ? Colors.white : Colors.black87)),
            const SizedBox(height: 8),
            Text('يرجى التحقق من اتصالك بالإنترنت', style: TextStyle(fontSize: 14, color: isDark ? Colors.grey[400] : Colors.grey[600])),
            const SizedBox(height: 24),
            ElevatedButton.icon(onPressed: _onRetry, icon: const Icon(Icons.refresh), label: const Text('إعادة المحاولة'), style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 32, vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)))),
          ],
        ),
      ),
    );
                ...
                children: [
                    ...
                    children: [
                      GestureDetector(
                        ...
                        child: CircleAvatar(
                          ...
                          child: Text(_isLoggedIn ? _userName[0].toUpperCase() : 'م', style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
                        ),
                      ),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Text(_isLoggedIn ? '${_getGreeting()}، $_userName 👋🏻' : 'مرحباً بك في صحتك', style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
                            Text(_isLoggedIn ? 'كيف تشعر اليوم؟' : 'سجل دخولك للاستفادة', style: TextStyle(color: Colors.white.withOpacity(0.8), fontSize: 13)),
                          ],
                        ),
                      ),
                      GestureDetector(
                        ...
                        child: Stack(
                          children: [
                            Container(padding: const EdgeInsets.all(8), child: Image.asset('assets/images/icons/top_bar/notifications.png', width: 26, height: 26, color: Colors.white, errorBuilder: (_, __, ___) => Icon(Icons.notifications, color: Colors.white, size: 26))),
                            if (_notificationCount > 0)
                              Positioned(
                                ...
                                child: Container(
                                  ...
                                  child: Text('$_notificationCount', style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
                                ),
                              ),
                          ],
                        ),
                      ...
                      GestureDetector(
                        ...
                        child: Container(padding: const EdgeInsets.all(8), child: Image.asset('assets/images/icons/top_bar/Shopping cart.png', width: 26, height: 26, color: Colors.white, errorBuilder: (_, __, ___) => Icon(Icons.shopping_cart, color: Colors.white, size: 26))),
                      ),
                    ],
                  ),
                  const SizedBox(height: 8),
                  GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(30), border: Border.all(color: Colors.white.withOpacity(0.3), width: 1)),
                      child: Row(
                        children: [
                          Image.asset('assets/images/icons/search/Search_button.png', width: 20, height: 20, errorBuilder: (_, __, ___) => Icon(Icons.search, color: Colors.white.withOpacity(0.7), size: 20)),
                          const SizedBox(width: 8),
                          Expanded(child: Text('ابحث عن طبيب، دواء، أو خدمة...', style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 14))),
                          Image.asset('assets/images/chat/microphone.png', width: 20, height: 20, errorBuilder: (_, __, ___) => Icon(Icons.mic, color: Colors.white.withOpacity(0.7), size: 20)),
                        ],
                      ),
                    ),
                  ),
  ...
  Widget _buildBannerCarousel(bool isDark) {
    if (_bannerImages.isEmpty) return Container(height: 180, decoration: BoxDecoration(color: isDark ? const Color(0xFF1A2540) : Colors.grey[200], borderRadius: BorderRadius.circular(16)), child: const Center(child: Text('لا توجد بانرات')));
    return Stack(
      children: [
        CarouselSlider(
          options: CarouselOptions(height: 180, autoPlay: true, autoPlayInterval: const Duration(seconds: 4), autoPlayAnimationDuration: const Duration(milliseconds: 800), autoPlayCurve: Curves.fastOutSlowIn, enlargeCenterPage: true, viewportFraction: 0.92, onPageChanged: (index, reason) => setState(() => _currentBanner = index)),
          items: _bannerImages.map((url) {
            return Container(
              ...
              child: ClipRRect(borderRadius: BorderRadius.circular(16), child: AppImage(imageUrl: url, height: 180, width: double.infinity, fit: BoxFit.cover)),
            );
          }).toList(),
        ),
        Positioned(
          ...
          child: Row(
            children: _bannerImages.asMap().entries.map((entry) {
              ...
              return AnimatedContainer(duration: const Duration(milliseconds: 300), width: isActive ? 20 : 8, height: 6, margin: const EdgeInsets.symmetric(horizontal: 4), decoration: BoxDecoration(color: isActive ? Colors.white : Colors.white.withOpacity(0.5), borderRadius: BorderRadius.circular(3)));
            }).toList(),
          ),
        ),
      ],
  ...
  Widget _buildStatsRow() {
    ...
    final isDark = Theme.of(context).brightness == Brightness.dark;
    return Row(children: statsData.map((stat) {
                ...
                children: [
                  SizedBox(
                    ...
                    child: Stack(
                      ...
                      children: [
                        CustomPaint(painter: CircularProgressPainter(progress: progress, backgroundColor: color.withOpacity(0.15), progressColor: color, strokeWidth: 5), child: const SizedBox(width: 60, height: 60)),
                        Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Icon(stat['icon'] as IconData, color: color, size: 18),
                            const SizedBox(height: 2),
                            Text(isInt ? (value * progress).toInt().toString() : (value * progress).toStringAsFixed(1), style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: color)),
                          ],
                        ),
                      ],
                    ),
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Image.asset(service['icon'] as String, width: 40, height: 40, errorBuilder: (_, __, ___) => Icon(Icons.image, color: isDark ? Colors.grey[400] : Colors.grey[600], size: 40)),
                  const SizedBox(height: 6),
                  Text(service['label'] as String, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: isDark ? Colors.grey[400] : Colors.grey[800]), textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis),
                ],
              ),
            ),
          );
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return GestureDetector(
          ...
          child: Container(
            decoration: BoxDecoration(color: isDark ? const Color(0xFF1A2540) : Colors.white, borderRadius: BorderRadius.circular(14), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 8, offset: const Offset(0, 2))]),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Stack(
                  children: [
                    ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(14)), child: Hero(tag: 'doctor_image_${doctor['id']}', child: AppImage(imageUrl: doctor['image'] as String, height: 90, width: double.infinity, fit: BoxFit.cover))),
                    Positioned(
                      top: 6,
                      right: 6,
                      child: Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: Colors.black.withOpacity(0.7), borderRadius: BorderRadius.circular(8)), child: Row(children: [const Icon(Icons.star, color: Colors.amber, size: 10), const SizedBox(width: 2), Text(doctor['rating'].toString(), style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold))])),
                    ),
                  ],
                ),
                Padding(
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          ...
                          style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 4), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6)), minimumSize: const Size(0, 28)),
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final priceAfterDiscount = hasDiscount ? (product['price'] as int) * (100 - (product['discount'] as int)) ~/ 100 : product['price'] as int;
          return GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(color: isDark ? const Color(0xFF1A2540) : Colors.white, borderRadius: BorderRadius.circular(14), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 8, offset: const Offset(0, 2))]),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.center,
                children: [
                  Stack(
                    children: [
                      ClipRRect(borderRadius: BorderRadius.circular(12), child: AppImage(imageUrl: product['image'] as String, height: 80, width: 80, fit: BoxFit.contain)),
                      if (hasDiscount) Positioned(top: 4, right: 4, child: Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: Colors.red, borderRadius: BorderRadius.circular(8)), child: Text('-${product['discount']}%', style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold)))),
                    ],
                  ),
                  const SizedBox(height: 8),
                  ...
                  const SizedBox(height: 4),
                  Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: AppColors.primary.withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: Text(product['category'] as String, style: const TextStyle(fontSize: 9, color: AppColors.primary))),
                  const Spacer(),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      if (hasDiscount) ...[Text('${product['price']} ر.ي', style: TextStyle(fontSize: 10, color: isDark ? Colors.grey[500] : Colors.grey[400], decoration: TextDecoration.lineThrough)), const SizedBox(width: 4)],
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                Stack(
                  children: [
                    ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(14)), child: Hero(tag: 'hospital_${hospital['id']}', child: AppImage(imageUrl: hospital['image'] as String, height: 90, width: double.infinity, fit: BoxFit.cover))),
                    Positioned(top: 6, right: 6, child: Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: Colors.black.withOpacity(0.7), borderRadius: BorderRadius.circular(8)), child: Row(children: [const Icon(Icons.star, color: Colors.amber, size: 10), const SizedBox(width: 2), Text(hospital['rating'].toString(), style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold))]))),
                    Positioned(bottom: 6, left: 6, child: Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: hospital['open'] == true ? Colors.green.withOpacity(0.9) : Colors.red.withOpacity(0.9), borderRadius: BorderRadius.circular(8)), child: Text(hospital['open'] == true ? 'مفتوح' : 'مغلق', style: const TextStyle(color: Colors.white, fontSize: 8)))),
                  ],
                ),
                Padding(
                  padding: const EdgeInsets.all(8),
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(14)), child: AppImage(imageUrl: lab['image'] as String, height: 90, width: double.infinity, fit: BoxFit.cover)),
                Padding(
                  padding: const EdgeInsets.all(8),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(14)), child: AppImage(imageUrl: pharmacy['image'] as String, height: 90, width: double.infinity, fit: BoxFit.cover)),
                Padding(
                  padding: const EdgeInsets.all(8),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(14)), child: AppImage(imageUrl: article['image'] as String, height: 80, width: double.infinity, fit: BoxFit.cover)),
                Padding(
                  padding: const EdgeInsets.all(8),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 1), decoration: BoxDecoration(color: AppColors.primary.withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: Text(article['category'] as String, style: TextStyle(fontSize: 8, color: AppColors.primary))),
                      const SizedBox(height: 4),
      ...
      itemBuilder: (context, index) {
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(color: AppColors.primary.withOpacity(0.08), borderRadius: BorderRadius.circular(14), border: Border.all(color: AppColors.primary.withOpacity(0.2))),
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Image.asset(tip['icon'] as String, width: 48, height: 48, errorBuilder: (_, __, ___) => Icon(Icons.image, color: isDark ? Colors.grey[400] : Colors.grey[600], size: 40)),
                const SizedBox(height: 8),
                ...
                const SizedBox(height: 4),
                Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: AppColors.primary.withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: Text('اقرأ المزيد', style: TextStyle(fontSize: 9, color: AppColors.primary))),
              ],
            ),
          ),
        );
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(item['label'] as String, style: TextStyle(fontSize: 10, color: isDark ? Colors.white : Colors.black87), textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis),
              ],
            ),
          ),
        );
  ...
  Widget _buildWeatherWidget() {
    return GestureDetector(
      ...
      child: Container(
        padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.cyan.withOpacity(0.1), Colors.cyan.withOpacity(0.05)]), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.cyan.withOpacity(0.2))),
        child: Row(
          children: [
            Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.cyan.withOpacity(0.1), shape: BoxShape.circle), child: Icon(_weatherData?['icon'] == 'sunny' ? Icons.wb_sunny : Icons.cloud, color: Colors.cyan, size: 20)),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('🌤️ ${_weatherData?['temp'] ?? '--'}°C', style: const TextStyle(fontWeight: FontWeight.bold)),
                  Text(_weatherData?['condition'] ?? 'غير معروف', style: const TextStyle(fontSize: 12, color: Colors.grey)),
                  if (_weatherData?['healthTip'] != null) Text('💡 ${_weatherData?['healthTip']}', style: const TextStyle(fontSize: 10, color: Colors.cyan)),
                ],
              ),
            ),
  ...
  Widget _buildAIRecommendation() {
    return GestureDetector(
      ...
      child: Container(
        padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.purple.withOpacity(0.1), Colors.purple.withOpacity(0.05)]), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.purple.withOpacity(0.2))),
        child: Row(
          children: [
            Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.purple.withOpacity(0.1), shape: BoxShape.circle), child: const Icon(Icons.auto_awesome, color: Colors.purple, size: 20)),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('🤖 توصيات مخصصة', style: TextStyle(fontWeight: FontWeight.bold)),
                  Text(_aiRecommendation?['title'] ?? 'اكتشف توصيات صحية لك', style: const TextStyle(fontSize: 12, color: Colors.grey)),
                ],
              ),
            ),
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  CircleAvatar(radius: 18, backgroundColor: AppColors.primary.withOpacity(0.1), child: Text(post['avatar'] as String, style: TextStyle(color: AppColors.primary, fontSize: 14, fontWeight: FontWeight.bold))),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(children: [Text(post['time'] as String, style: TextStyle(fontSize: 12, color: isDark ? Colors.grey[400] : Colors.grey[600])), const SizedBox(width: 8), Container(padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 1), decoration: BoxDecoration(color: Colors.grey.withOpacity(0.2), borderRadius: BorderRadius.circular(4)), child: Text('👁️ ${post['views']}', style: TextStyle(fontSize: 9, color: isDark ? Colors.grey[400] : Colors.grey[600]))]),
                      ],
                    ),
                  ),
                  PopupMenuButton(
                    ...
                    itemBuilder: (context) => [
                      PopupMenuItem(child: Row(children: [Icon(post['saved'] == true ? Icons.bookmark : Icons.bookmark_border, color: AppColors.primary, size: 16), const SizedBox(width: 8), Text(post['saved'] == true ? 'إلغاء الحفظ' : 'حفظ')]), onTap: () => _savePost(index)),
                      PopupMenuItem(child: Row(children: [const Icon(Icons.flag, color: Colors.red, size: 16), const SizedBox(width: 8), Text(post['reported'] == true ? 'تم الإبلاغ' : 'إبلاغ')]), onTap: () => _reportPost(index)),
                    ],
                  ),
                ],
              ),
              ...
              if (post['image'] != null)
                GestureDetector(
                  onTap: () => showDialog(context: context, builder: (_) => Dialog(child: AppImage(imageUrl: post['image'] as String, width: double.infinity, height: 300, fit: BoxFit.contain)))),
                  child: ClipRRect(borderRadius: BorderRadius.circular(10), child: AppImage(imageUrl: post['image'] as String, height: 200, width: double.infinity, fit: BoxFit.contain)),
                ),
              ...
              Row(
                children: [
                  GestureDetector(
                    ...
                    child: Row(
                      children: [
                        Icon(post['liked'] == true ? Icons.favorite : Icons.favorite_border, color: post['liked'] == true ? AppColors.primary : (isDark ? Colors.grey[400] : Colors.grey[600]), size: 18),
                  ...
                  GestureDetector(
                    ...
                    child: Icon(post['saved'] == true ? Icons.bookmark : Icons.bookmark_border, color: post['saved'] == true ? AppColors.primary : (isDark ? Colors.grey[400] : Colors.grey[600]), size: 18),
                  ),
                ],
              ),
            ],
