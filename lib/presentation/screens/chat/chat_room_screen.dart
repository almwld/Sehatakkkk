...
class _ChatRoomScreenState extends State<ChatRoomScreen> {
  ...
  void _listenToTyping() {
    _typingSubscription = _firestore
      ...
      final Map typing = rawTyping is Map ? rawTyping : <dynamic, dynamic>{};
  ...
  // 📞 بدء المكالمة
  // ============================================================
  // CallService هو المصدر الوحيد لإنشاء سجل المكالمة ورسالة النظام.
  // نمرر chatId الحقيقي فقط؛ CallScreen يتولى initiateCall عند الحاجة.
            ...
            onPressed: () async {
              ...
              try {
                ...
                for (int start = 0; start < docs.length; start += 450) {
                  final end = (start + 450 < docs.length) ? start + 450 : docs.length;
      ...
      body: ChatBackground(
        child: Column(
          children: [
            ...
            if (_typingUsers.isNotEmpty) TypingIndicator(name: _typingUsers.first),
            Expanded(
              child: BlocListener<MessagesBloc, MessagesState>(
                listener: (context, state) {
                  if (state is MessagesLoaded && !state.isLoadingMore) {
                ...
                child: BlocBuilder<MessagesBloc, MessagesState>(
                  builder: (context, state) {
                    ...
                    if (state is MessagesError) return _buildErrorState(isDark);
                    if (state is MessagesLoaded) {
                      final messages = state.messages;
                      if (messages.isEmpty) return _buildEmptyState(isDark);
            ...
            ChatInputBar(
              ...
              onSendImage: (path) => _sendImage(path),
