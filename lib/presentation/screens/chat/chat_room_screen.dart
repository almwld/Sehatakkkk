...
class _ChatRoomScreenState extends State<ChatRoomScreen> {
  ...
  void _startPendingRefresh() {
    ...
    void schedule() {
      _pendingRefreshTimer = Timer(delay, () async {
        ...
        final hasPending = _localMedia.isNotEmpty;
        delay = hasPending ? Duration(seconds: (delay.inSeconds * 2).clamp(2, 16)) : const Duration(seconds: 8);
        schedule();
      });
    }
    schedule();
  ...
  void _listen() {
    _chatSub = _firestore.collection('chats').doc(widget.chatId).snapshots().listen((snapshot) {
      ...
      final uid = _auth.currentUser?.uid; final mutedFor = data['mutedFor']; final pinnedFor = data['pinnedFor'];
      setState(() { _muted = uid != null && mutedFor is Map && mutedFor[uid] == true ? true : (mutedFor is! Map && data['isMuted'] == true); _pinned = uid != null && pinnedFor is Map && pinnedFor[uid] == true ? true : (pinnedFor is! Map && data['isPinned'] == true); });
    });
    ...
    return Scaffold(
      ...
      body: Column(children: [
        Expanded(child: _loading && all.isEmpty ? const Center(child: CircularProgressIndicator()) : all.isEmpty ? const Center(child: Text('ابدأ المحادثة')) : ChatBackground(child: ListView.builder(reverse: true, padding: const EdgeInsets.all(8), itemCount: all.length, itemBuilder: (_, index) {
          final message = all[index]; final remote = message['isLocal'] != true; final messageId = message['id']?.toString(); final model = remote && messageId != null ? _messages.firstWhere((m) => m.id == messageId, orElse: () => MessageModel(id: '', chatId: '', senderId: '', senderName: '')) : null;
          return MessageBubble(key: ValueKey(message['id'] ?? index), message: message, isMe: message['senderId'] == _auth.currentUser?.uid || message['isLocal'] == true, onReply: model == null || model.id.isEmpty ? null : () => _startReply(model), onDelete: model == null || model.id.isEmpty ? null : () => _deleteMessage(model), onCallAgain: (_) => _call(false), onReaction: remote && messageId != null ? (emoji) => _chat.addReaction(widget.chatId, messageId, emoji) : null);
        }))),
