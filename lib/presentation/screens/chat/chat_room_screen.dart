...
class ChatRoomScreen extends StatefulWidget {
  final String chatId; final String otherUserId; final String otherUserName; final String? otherUserImage; final bool isGroup; final String? groupImage; final String? lastMessage;
...
class _ChatRoomScreenState extends State<ChatRoomScreen> {
  final _firestore = FirebaseFirestore.instance; final _auth = FirebaseAuth.instance; final _chat = ChatService();
  StreamSubscription<QuerySnapshot<Map<String, dynamic>>>? _messagesSub; StreamSubscription<DocumentSnapshot<Map<String, dynamic>>>? _chatSub; StreamSubscription<DocumentSnapshot<Map<String, dynamic>>>? _userSub; Timer? _pendingRefreshTimer;
  List<MessageModel> _messages = []; final List<Map<String, dynamic>> _localMedia = []; bool _loading = true; bool _online = false; bool _muted = false; bool _pinned = false;
  ...
  void _startPendingRefresh() { _pendingRefreshTimer?.cancel(); _pendingRefreshTimer = Timer.periodic(const Duration(seconds: 1), (_) { if (mounted) unawaited(_loadPendingMedia()); }); }
  ...
  void _listen() {
    _chatSub = _firestore.collection('chats').doc(widget.chatId).snapshots().listen((snapshot) { if (!mounted || !snapshot.exists) return; final data = snapshot.data() ?? <String, dynamic>{}; final uid = _auth.currentUser?.uid; final mutedFor = data['mutedFor']; final pinnedFor = data['pinnedFor']; setState(() { _muted = uid != null && mutedFor is Map && mutedFor[uid] == true ? true : (mutedFor is! Map && data['isMuted'] == true); _pinned = uid != null && pinnedFor is Map && pinnedFor[uid] == true ? true : (pinnedFor is! Map && data['isPinned'] == true); }); });
    ...
    _messagesSub = _messagesRef.orderBy('timestamp', descending: true).limit(100).snapshots().listen((snapshot) { if (!mounted) return; final uid = _auth.currentUser?.uid; final messages = snapshot.docs.map((doc) => MessageModel.fromFirestore(doc.id, doc.data())).where((m) { final raw = m.toFirestore()['deletedFor']; return uid == null || raw is! Map || raw[uid] != true; }).toList(); final remoteIds = messages.map((m) => m.id).toSet(); setState(() { _messages = messages; _localMedia.removeWhere((m) => remoteIds.contains(m['id'])); _loading = false; }); _markRead(); unawaited(_loadPendingMedia()); }, onError: (error) { debugPrint('chat stream: $error'); if (mounted) setState(() => _loading = false); });
  }
  ...
  @override void dispose() { _pendingRefreshTimer?.cancel(); _messagesSub?.cancel(); _chatSub?.cancel(); _userSub?.cancel(); super.dispose(); }
  @override Widget build(BuildContext context) {
    final dark = Theme.of(context).brightness == Brightness.dark; final image = widget.otherUserImage ?? widget.groupImage;
    final all = <Map<String, dynamic>>[..._localMedia, ..._messages.map((m) => m.toFirestore()..['id'] = m.id)]; all.sort((a, b) => _messageTime(b['timestamp']).compareTo(_messageTime(a['timestamp'])));
    return Scaffold(backgroundColor: dark ? const Color(0xFF0B1121) : const Color(0xFFF2F5F6), appBar: AppBar(elevation: 0, backgroundColor: dark ? const Color(0xFF101827) : Colors.white, leading: const BackButton(), titleSpacing: 0, title: InkWell(onTap: _profile, child: Row(children: [CircleAvatar(radius: 21, backgroundColor: AppColors.primary.withOpacity(.12), backgroundImage: image != null ? CachedNetworkImageProvider(image) : null, child: image == null ? Text(widget.otherUserName.isEmpty ? 'م' : widget.otherUserName.substring(0, 1)) : null), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(widget.isGroup ? 'المجموعة' : widget.otherUserName, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700)), Text(_online ? 'متصل الآن' : 'غير متصل', style: TextStyle(fontSize: 11, color: _online ? Colors.green : Colors.grey))]))])), actions: [if (!widget.isGroup) IconButton(onPressed: () => _call(false), icon: const Icon(Icons.call_rounded)), if (!widget.isGroup) IconButton(onPressed: () => _call(true), icon: const Icon(Icons.videocam_rounded)), PopupMenuButton<String>(onSelected: (value) { if (value == 'mute') _toggleMute(); if (value == 'pin') _togglePin(); }, itemBuilder: (_) => [PopupMenuItem(value: 'mute', child: Text(_muted ? 'إلغاء كتم الإشعارات' : 'كتم الإشعارات')), PopupMenuItem(value: 'pin', child: Text(_pinned ? 'إلغاء تثبيت المحادثة' : 'تثبيت المحادثة'))])]), body: Column(children: [Expanded(child: _loading && all.isEmpty ? const Center(child: CircularProgressIndicator()) : all.isEmpty ? const Center(child: Text('ابدأ المحادثة')) : ChatBackground(child: ListView.builder(reverse: true, padding: const EdgeInsets.all(8), itemCount: all.length, itemBuilder: (_, index) { final message = all[index]; final remote = message['isLocal'] != true; final messageId = message['id']?.toString(); final model = remote && messageId != null ? _messages.firstWhere((m) => m.id == messageId, orElse: () => MessageModel(id: '', chatId: '', senderId: '', senderName: '')) : null; return MessageBubble(key: ValueKey(message['id'] ?? index), message: message, isMe: message['senderId'] == _auth.currentUser?.uid || message['isLocal'] == true, onDelete: model == null || model.id.isEmpty ? null : () => _deleteMessage(model), onCallAgain: (_) => _call(false), onReaction: remote && messageId != null ? (emoji) => _chat.addReaction(widget.chatId, messageId, emoji) : null); })) ), ChatInputBar(chatId: widget.chatId, onSendMessage: (_) {}, onSendImage: (_) {}, onLocalMedia: _addLocalMedia, onShareLocation: _shareLocation)]));
  }
}
