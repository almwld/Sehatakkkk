...
class _ChatInputBarState extends State<ChatInputBar> {
  ...
  Future<void> _loadRecentGallery() async {
    ...
    try {
      ...
      if (!permission.isAuth) {
        if (mounted) ToastService.showError('اسمح بالوصول إلى الصور لعرض الملفات الحديثة.');
        return;
      }
  ...
  Future<void> _pickFile() async {
    ...
    await _sendMedia(File(picked!.path!), type: 'file', folder: 'files', preview: '📎 ${picked.name}', name: picked.name, size: _formatBytes(picked.size), mime: picked.extension == null ? null : _guessMime(picked.extension!));
  }
  ...
  Future<void> _startRecording() async {
    ...
    try {
      ...
      setState(() { _recording = true; _paused = false; _recordPath = path; });
  ...
  Future<void> _deleteRecording() async {
    ...
    final path = _recordPath; _recordPath = null;
  ...
  Future<void> _sendRecording() async {
    ...
    final path = _recordPath; if (path == null) return;
    final file = File(path);
    if (!await file.exists() || await file.length() < 1000) { await _deleteRecording(); ToastService.showError('التسجيل قصير جدًا.'); return; }
    setState(() => _sending = true);
    try {
      await _enqueueMedia(file, type: 'audio', folder: 'audio', preview: '🎤 رسالة صوتية', name: file.path.split(Platform.pathSeparator).last, mime: 'audio/mp4', audioDuration: _duration.inSeconds.toString());
      _recordPath = null; _duration = Duration.zero;
    } catch (e) { debugPrint('audio enqueue: $e'); ToastService.showError('تعذر تجهيز التسجيل للإرسال.'); }
    finally { try { if (await file.exists()) await file.delete(); } catch (_) {} if (mounted) setState(() => _sending = false); }
  }
    ...
    return Column(mainAxisSize: MainAxisSize.min, children: [
      AnimatedSwitcher(
        ...
        transitionBuilder: (child, animation) => SizeTransition(
          ...
          axisAlignment: 1,
          child: FadeTransition(opacity: animation, child: SlideTransition(
        ...
        child: _attachments ? _mediaMenu(dark) : const SizedBox.shrink(key: ValueKey('closed')),
      ),
      Material(color: dark ? const Color(0xFF121A29) : Colors.white, child: SafeArea(top: false, child: Padding(padding: const EdgeInsets.fromLTRB(8, 6, 8, 6), child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
        ...
        GestureDetector(onTap: _hasText ? _sendText : _startRecording, child: CircleAvatar(radius: 23, backgroundColor: _sending ? Colors.grey : AppColors.primary, child: Icon(_hasText ? Icons.send_rounded : Icons.mic_rounded, color: Colors.white, size: 21))),
      ]))))),
    ]);
  }

  Widget _voiceBar(bool dark) => Material(color: dark ? const Color(0xFF121A29) : Colors.white, child: SafeArea(top: false, child: Padding(padding: const EdgeInsets.all(6), child: Row(children: [
  ...
  Widget _mediaMenu(bool dark) => Material(
    ...
    child: Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        if (_loadingRecent) const SizedBox(height: 74, child: Center(child: SizedBox(width: 24, height: 24, child: CircularProgressIndicator(strokeWidth: 2))))
        else if (_recentAssets.isNotEmpty) _recentGallery(dark)
  ...
  Widget _recentGallery(bool dark) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Padding(padding: const EdgeInsetsDirectional.only(start: 4, bottom: 7), child: Row(children: [
      Icon(Icons.history_rounded, size: 17, color: AppColors.primary),
      const SizedBox(width: 5),
      Text('الملفات الحديثة', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: dark ? Colors.white : const Color(0xFF263238))),
    ])),
          ...
          builder: (context, snapshot) {
            return GestureDetector(
              ...
              child: ClipRRect(
                ...
                child: SizedBox(width: 76, height: 76, child: Stack(fit: StackFit.expand, children: [
                  if (snapshot.hasData) Image.memory(snapshot.data!, fit: BoxFit.cover) else Container(color: dark ? Colors.white10 : Colors.black12, child: const Center(child: CircularProgressIndicator(strokeWidth: 1.5))),
                  if (asset.type == AssetType.video) Positioned(bottom: 5, end: 5, child: Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 2), decoration: BoxDecoration(color: Colors.black54, borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.play_arrow, color: Colors.white, size: 14))),
                ])),
              ),
            );
          },
