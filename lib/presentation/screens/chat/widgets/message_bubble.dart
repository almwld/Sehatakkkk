...
class _MessageBubbleState extends State<MessageBubble> {
  ...
  void initState() {
    ...
    if (widget.message['type'] == 'audio' &&
        (widget.message['audioUrl'] ?? '').toString().isNotEmpty) {
    ...
    return GestureDetector(
      ...
      child: Container(
        decoration: BoxDecoration(
          color: widget.isMe
              ? AppColors.primary
              : (dark ? const Color(0xFF1A2540) : Colors.grey[100]),
  ...
  Widget _buildContent(String type, bool dark) {
    switch (type) {
      case 'image':
        return _buildImage(widget.message['imageUrl']?.toString() ?? '', dark);
      case 'video':
        return _buildVideo(
          widget.message['videoUrl']?.toString() ??
              widget.message['fileUrl']?.toString() ?? '',
          dark,
        );
      case 'audio':
        return _buildAudio(widget.message, dark);
      case 'file':
        return _buildFile(widget.message, dark);
      case 'location':
        return _buildLocation(widget.message, dark);
      case 'system':
        return _buildSystem(widget.message);
      default:
        return _buildText(widget.message, dark);
    }
  }
  ...
  Widget _buildImage(String url, bool dark) {
    if (url.isEmpty) {
      return _buildText({'text': 'تعذر تحميل الصورة'}, dark);
    }
  ...
  Widget _buildVideo(String url, bool dark) {
    if (url.isEmpty) {
      return _buildText({'text': 'تعذر تحميل الفيديو'}, dark);
    }
              ...
              child: Column(
                ...
                children: [
                  Container(
                    height: 4,
                    decoration: BoxDecoration(
                      color: widget.isMe ? Colors.white38 : Colors.black12,
  ...
  Future<void> _toggleAudio(String url) async {
    if (url.isEmpty || _audioPlayer == null) return;
    try {
      if (_isPlaying) {
      ...
      ToastService.showError('تعذر تشغيل التسجيل الصوتي');
  ...
  Widget _buildFile(Map<String, dynamic> m, bool dark) {
    ...
    final name = rawName.isNotEmpty ? rawName : 'ملف';
      ...
      InkWell(
        onTap: () async {
          ...
          if (uri != null && await canLaunchUrl(uri)) {
            ...
            ToastService.showError('تعذر فتح الملف');
          }
        },
        child: Padding(
  ...
  Widget _buildLocation(Map<String, dynamic> m, bool dark) {
    final label = m['locationAddress']?.toString().trim() ?? '';
    final text = label.isNotEmpty ? label : (m['text']?.toString() ?? 'الموقع');
  ...
  Widget _buildSystem(Map<String, dynamic> m) {
    return Padding(
