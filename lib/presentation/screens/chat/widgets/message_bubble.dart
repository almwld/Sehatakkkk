...
class _MessageBubbleState extends State<MessageBubble> {
          ...
          child: Stack(
            ...
            children: [
              ...
              if (progress != null && progress >= 0 && progress < 1)
                Positioned(left: 8, right: 8, bottom: 3, child: LinearProgressIndicator(value: progress, minHeight: 2)),
              if (error)
            ...
            children: [
              if ((m['replyPreview']?.toString().trim() ?? '').isNotEmpty)
                Container(
                  ...
                  child: Text(m['replyPreview'].toString(), maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 11, color: widget.isMe ? Colors.white70 : Colors.black54)),
                ),
              Row(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.end,
                children: [
                  Flexible(child: Text(m['text']?.toString() ?? '', style: TextStyle(color: widget.isMe ? Colors.white : (dark ? Colors.white : Colors.black87), fontSize: 14))),
  ...
  Widget _buildImage(String path) {
    ...
    return GestureDetector(
      onTap: () => showDialog<void>(context: context, barrierColor: Colors.black87, builder: (_) => Dialog(backgroundColor: Colors.transparent, child: InteractiveViewer(child: image))),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(14),
        child: local
            ? Image.file(File(cleanPath), width: 230, height: 230, fit: BoxFit.cover)
            : CachedNetworkImage(imageUrl: path, width: 230, height: 230, fit: BoxFit.cover, placeholder: (_, __) => const SizedBox(width: 230, height: 230, child: Center(child: CircularProgressIndicator(strokeWidth: 2))), errorWidget: (_, __, ___) => const SizedBox(width: 230, height: 230, child: Center(child: Icon(Icons.broken_image)))),
      ),
    );
  }
  ...
  Widget _buildVideo(String path) {
    ...
    return GestureDetector(
      ...
      child: Container(width: 230, height: 160, decoration: BoxDecoration(color: Colors.black, borderRadius: BorderRadius.circular(14)), child: const Center(child: Icon(Icons.play_circle_fill, color: Colors.white, size: 50))),
    );
  }
  ...
  Widget _buildFile(Map<String, dynamic> m, bool dark) {
    ...
    return _shell(InkWell(
      onTap: () async {
        if (url.isEmpty) return;
        final target = _isLocal(url) ? Uri.file(url.replaceFirst('file://', '')) : Uri.tryParse(url);
        if (target != null && await canLaunchUrl(target)) await launchUrl(target, mode: LaunchMode.externalApplication);
      },
      child: Padding(padding: const EdgeInsets.all(12), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.insert_drive_file, color: tc, size: 30), const SizedBox(width: 10), Flexible(child: Text(name, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(color: tc, fontWeight: FontWeight.w600))), const SizedBox(width: 8), Icon(Icons.download_for_offline, color: tc)])),
    ), dark);
  }
  ...
  void _options() {
    showModalBottomSheet<void>(
      ...
      builder: (_) => SafeArea(child: Wrap(children: [
        if (widget.onReaction != null) Padding(padding: const EdgeInsets.symmetric(vertical: 8), child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: ['👍', '❤️', '😂', '😮', '🙏'].map((emoji) => IconButton(onPressed: () { Navigator.pop(context); widget.onReaction?.call(emoji); }, icon: Text(emoji, style: const TextStyle(fontSize: 24))).toList())),
