...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _subscribeChats() {
    try {
      _chatsSubscription = _chatService.streamChats().listen((items) {
        if (mounted) setState(() { _chats = items; _loadingChats = false; });
      }, onError: (e) {
  ...
  @override
  void dispose() { _chatsSubscription?.cancel(); _searchController.dispose(); super.dispose(); }

  List<ChatModel> get _filteredChats => _chats.where((chat) {
    if (_search.isEmpty) return true;
    final uid = FirebaseAuth.instance.currentUser?.uid ?? '';
    return chat.getDisplayName(uid).toLowerCase().contains(_search) || (chat.lastMessage ?? '').toLowerCase().contains(_search);
  }).toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(backgroundColor: AppColors.primary, onPressed: () => _showDoctorsForNewChat(isDark), icon: const Icon(Icons.add_comment, color: Colors.white), label: const Text('محادثة جديدة', style: TextStyle(color: Colors.white))),
    );
  }
  ...
  Widget _buildChatList(bool isDark) {
    ...
    if (chats.isEmpty) return _buildInlineEmpty(_search.isEmpty ? 'لا توجد محادثات بعد\nابدأ محادثة مع طبيب موثّق' : 'لا توجد نتائج مطابقة', isDark);
    return ListView.separated(padding: const EdgeInsets.fromLTRB(16, 8, 16, 100), itemCount: chats.length, separatorBuilder: (_, __) => const SizedBox(height: 8), itemBuilder: (_, i) => _buildChatTile(chats[i], isDark));
  }
  ...
  Widget _buildChatTile(ChatModel chat, bool isDark) {
    ...
    return Card(elevation: 0, color: isDark ? const Color(0xFF162039) : Colors.white, child: ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), leading: CircleAvatar(radius: 27, backgroundColor: AppColors.primary.withOpacity(.12), backgroundImage: image.isNotEmpty ? NetworkImage(image) : null, child: image.isEmpty ? const Icon(Icons.person, color: AppColors.primary) : null), title: Text(name, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text(chat.lastMessage?.isNotEmpty == true ? chat.lastMessage! : 'اضغط لفتح المحادثة', maxLines: 1, overflow: TextOverflow.ellipsis), trailing: unread > 0 ? CircleAvatar(radius: 12, backgroundColor: AppColors.primary, child: Text('$unread', style: const TextStyle(color: Colors.white, fontSize: 11))) : const Icon(Icons.chevron_left), onTap: otherId.isEmpty ? null : () => Navigator.push(context, MaterialPageRoute(builder: (_) => ChatRoomScreen(chatId: chat.id, doctorId: otherId, doctorName: name, doctorImage: image))));
  }

  Widget _buildDoctorCard(DoctorModel d, bool isDark) => Card(elevation: 0, color: isDark ? const Color(0xFF162039) : Colors.white, child: ListTile(leading: CircleAvatar(backgroundColor: AppColors.primary.withOpacity(.1), backgroundImage: (d.photoUrl ?? '').isNotEmpty ? NetworkImage(d.photoUrl!) : null, child: (d.photoUrl ?? '').isEmpty ? const Icon(Icons.person, color: AppColors.primary) : null), title: Text(d.name, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text('${d.specialty ?? 'طبيب'} • ⭐ ${(d.rating ?? 0).toStringAsFixed(1)}'), trailing: const Icon(Icons.chat_bubble_outline, color: AppColors.primary), onTap: () => _openChat(d)));
  ...
  Future<void> _openChat(DoctorModel doctor) async {
    ...
    } catch (e) { ToastService.showError('تعذر فتح المحادثة'); debugPrint('open chat error: $e'); }
  }
