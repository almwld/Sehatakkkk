...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _subscribeChats() {
    ...
    try {
      _subscription = _chatService.streamChats().listen((items) {
        if (!mounted) return;
        setState(() {
          _chats = items;
          _loadingChats = false;
  ...
  void _subscribeStatuses() {
    ...
    try {
      _statusSubscription = _statusService.streamActiveStatuses().listen((items) {
        if (!mounted) return;
        setState(() {
          _statuses = items;
  ...
  void _selectTab(int index) {
    ...
    if (_tabPageController.hasClients) {
      _tabPageController.animateToPage(index, duration: const Duration(milliseconds: 260), curve: Curves.easeOutCubic);
    }
  }

  void _onTabPageChanged(int index) {
    if (mounted && _tabIndex != index) setState(() => _tabIndex = index);
  }
  ...
  List<ChatModel> get _filteredChats {
    ...
    return _chats.where((chat) => chat.getDisplayName(uid).toLowerCase().contains(_search) || (chat.lastMessage ?? '').toLowerCase().contains(_search)).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        elevation: 0,
        bottom: PreferredSize(preferredSize: const Size.fromHeight(58), child: _buildTabs(isDark)),
      ),
      body: PageView(controller: _tabPageController, onPageChanged: _onTabPageChanged, children: [_buildConversationsTab(isDark), const CallsScreen(), _buildContactsTab(isDark)]),
      floatingActionButton: (_tabIndex == 0 || _tabIndex == 2)
          ? FloatingActionButton.extended(
              ...
              icon: Image.asset('assets/images/chat/attach_file.png', width: 21, height: 21, color: Colors.white, errorBuilder: (_, __, ___) => const SizedBox.shrink()),
  ...
  Widget _buildTabs(bool isDark) {
    ...
    return Container(
      ...
      height: 48,
      decoration: BoxDecoration(color: isDark ? const Color(0xFF162039) : Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: isDark ? Colors.white10 : const Color(0xFFE6EEEE))),
      child: Row(children: List.generate(labels.length, (index) {
        ...
        return Expanded(child: GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            child: Center(child: AnimatedDefaultTextStyle(duration: const Duration(milliseconds: 180), style: TextStyle(fontSize: 13, fontWeight: selected ? FontWeight.w700 : FontWeight.w500, color: selected ? Colors.white : (isDark ? Colors.white70 : const Color(0xFF526467))), child: Text(labels[index]))),
          ),
        ));
      })),
    );
  ...
  Widget _buildConversationsTab(bool isDark) {
    return Column(children: [
      Padding(padding: const EdgeInsets.fromLTRB(16, 14, 16, 8), child: TextField(controller: _searchController, textDirection: TextDirection.rtl, decoration: InputDecoration(hintText: 'ابحث في محادثاتك...', prefixIcon: const Icon(Icons.search), suffixIcon: _search.isEmpty ? null : IconButton(icon: const Icon(Icons.clear), onPressed: _searchController.clear), filled: true, fillColor: isDark ? const Color(0xFF162039) : Colors.white, border: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: BorderSide.none)))),
      Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 5), child: _buildAIAssistantCard(isDark)),
      Expanded(child: _buildChatList(isDark)),
    ]);
  }

  Widget _buildAIAssistantCard(bool isDark) {
    return Card(elevation: 0, color: isDark ? const Color(0xFF162039) : Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), child: ListTile(
      leading: Image.asset('assets/images/services/ai_assistant.png', width: 46, height: 46, fit: BoxFit.contain, errorBuilder: (_, __, ___) => const Icon(Icons.smart_toy_outlined, color: AppColors.primary, size: 30)),
  ...
  Widget _buildChatList(bool isDark) {
    ...
    if (chats.isEmpty) return Center(child: Text(_search.isEmpty ? 'لا توجد محادثات بعد\nابدأ محادثة مع طبيب من تبويب جهات الاتصال' : 'لا توجد نتائج مطابقة', textAlign: TextAlign.center, style: TextStyle(color: isDark ? Colors.white70 : Colors.black54)));
    return ListView.separated(padding: const EdgeInsets.fromLTRB(16, 8, 16, 100), itemCount: chats.length, separatorBuilder: (_, __) => const SizedBox(height: 8), itemBuilder: (_, i) => _chatTile(chats[i], isDark));
  }
  ...
  Widget _buildContactsTab(bool isDark) => HealthContactsSection(isDark: isDark);

  Future<void> _openAddStatus() async {
    if (!mounted) return;
    final created = await context.push<bool>(AppRouter.addStatus);
    if (created == true && mounted) _subscribeStatuses();
  }
  ...
  Future<void> _openExistingChat(String chatId, String otherId, String name, String image) async {
    ...
    try {
      ...
      if (mounted) await _chatService.markAsRead(chatId);
    } catch (e) {
  ...
  Future<void> _openChat(DoctorModel doctor) async {
    ...
    setState(() => _openingChat = true);
    try {
      final chatId = await _chatService.createChat(doctorId: doctorId, doctorName: doctor.name, doctorImage: doctor.photoUrl, patientName: user.displayName ?? 'مستخدم', patientImage: user.photoURL);
      if (!mounted || chatId.trim().isEmpty) return;
      await _chatService.markAsRead(chatId);
      if (!mounted) return;
      await context.push(AppRouter.chatRoom, extra: <String, dynamic>{'chatId': chatId, 'otherUserId': doctorId, 'otherUserName': doctor.name, 'otherUserImage': doctor.photoUrl, 'isGroup': false});
    } catch (e) {
  ...
  Future<void> _showDoctorsForNewChat(bool isDark) async {
    ...
    await showModalBottomSheet<void>(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (sheetContext) => Container(
      height: MediaQuery.of(sheetContext).size.height * .72,
      decoration: BoxDecoration(color: isDark ? const Color(0xFF0F172A) : Colors.white, borderRadius: const BorderRadius.vertical(top: Radius.circular(24))),
      child: Column(children: [
        const SizedBox(height: 10),
        Container(width: 42, height: 4, decoration: BoxDecoration(color: Colors.grey, borderRadius: BorderRadius.circular(4))),
        const Padding(padding: EdgeInsets.all(16), child: Align(alignment: Alignment.centerRight, child: Text('اختر طبيباً لبدء المحادثة', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
        Expanded(child: ListView.separated(padding: const EdgeInsets.fromLTRB(16, 0, 16, 24), itemCount: doctors.length, separatorBuilder: (_, __) => const SizedBox(height: 8), itemBuilder: (_, i) => _buildDoctorPickerTile(doctors[i], isDark, sheetContext))),
      ]),
    ));
  }
  ...
  Widget _buildDoctorPickerTile(DoctorModel doctor, bool isDark, BuildContext sheetContext) {
    ...
    return Card(elevation: 0, color: isDark ? const Color(0xFF162039) : const Color(0xFFF7FAFA), child: ListTile(
