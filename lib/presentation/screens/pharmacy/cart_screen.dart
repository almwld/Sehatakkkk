...
class _CartScreenState extends State<CartScreen> {
  ...
  Future<void> _checkout() async {
    ...
    try {
      final result = await _cart.checkout(deliveryFee: 0, deliveryAddress: _delivery ? _address.text.trim() : null, deliveryRequired: _delivery);
      if (!mounted) return;
      await showDialog<void>(context: context, builder: (_) => AlertDialog(title: const Text('تم الدفع بنجاح'), content: Text('رقم الطلب: ${result['orderId']}\nالإجمالي: ${result['total']} ر.ي\nرقم الفاتورة: ${result['invoiceId'] ?? '—'}'), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('متابعة'))]));
      setState(() {});
