  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        return snapshot.data?.data()?['role'] == 'doctor' ? const _DoctorDashboard() : const PatientDashboard();
      },
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        FutureBuilder<QuerySnapshot<Map<String, dynamic>>>(future: uid == null ? null : FirebaseFirestore.instance.collection('appointments').where('doctorId', isEqualTo: uid).limit(50).get(), builder: (_, snap) => Row(children: [Expanded(child: _stat('المواعيد', '${snap.data?.docs.length ?? 0}', 'assets/images/chat/calendar_booking.png', dark)), const SizedBox(width: 10), Expanded(child: _stat('الحالة', 'نشط', 'assets/images/services/medical_community.png', dark))])),
        const SizedBox(height: 14),
        _action(context, 'assets/images/chat/calendar_booking.png', 'المواعيد', 'متابعة المواعيد والحجوزات', AppRouter.consultation),
        _action(context, 'assets/images/services/medical_community.png', 'مجتمع صحتك', 'إدارة المنشورات الطبية', AppRouter.more),
