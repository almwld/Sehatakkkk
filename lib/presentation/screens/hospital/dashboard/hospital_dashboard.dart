...
class _HospitalDashboardState extends State<HospitalDashboard> with SingleTickerProviderStateMixin {
  ...
  @override void initState() { super.initState(); _tabController = TabController(length: 4, vsync: this); _loadData(); }
  ...
  Future<void> _loadRevenue(String providerId) async {
    try {
      ...
      final total = snap.docs.fold<double>(0, (sum, doc) => sum + ((doc.data()['amount'] as num?)?.toDouble() ?? 0));
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        Container(padding: const EdgeInsets.all(16), decoration: const BoxDecoration(color: AppColors.primary, borderRadius: BorderRadius.vertical(bottom: Radius.circular(24))), child: Row(children: [_buildStatCard('المرضى', '${_bookings.length}', Icons.people, Colors.blue), _buildStatCard('الحجوزات', '${_bookings.length}', Icons.event, Colors.amber), _buildStatCard('الإيرادات', '${_totalRevenue.toStringAsFixed(0)} ريال', Icons.attach_money, Colors.green), _buildStatCard('هذا الشهر', '${_totalRevenue.toStringAsFixed(0)} ريال', Icons.calendar_month, Colors.purple)])),
  ...
  Widget _buildDepartmentsTab() { final departments = ['الطوارئ', 'الباطنية', 'الجراحة', 'الأطفال', 'النساء والولادة']; return ListView.builder(padding: const EdgeInsets.all(16), itemCount: departments.length, itemBuilder: (_, index) => ListTile(leading: const Icon(Icons.medical_services, color: AppColors.primary), title: Text(departments[index]), subtitle: const Text('القسم متاح'), trailing: Switch(value: true, onChanged: (_) {}))); }
  Widget _buildStatisticsTab() => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.bar_chart, size: 64, color: Colors.grey), const SizedBox(height: 16), Text('إجمالي الحجوزات: ${_bookings.length}'), const SizedBox(height: 8), Text('إجمالي الإيرادات: ${_totalRevenue.toStringAsFixed(0)} ريال')]));
