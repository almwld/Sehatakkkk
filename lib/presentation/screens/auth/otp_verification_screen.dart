...
class _OtpVerificationScreenState extends State<OtpVerificationScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: 'رمز التحقق'),
      body: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
          ...
          Text('تم إرسال رمز إلى ${widget.phone}', style: TextStyle(color: Colors.grey, fontSize: 14)),
          const SizedBox(height: 32),
          Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: List.generate(6, (i) => SizedBox(width: 48, height: 56, child: TextField(controller: _ctrls[i], focusNode: _fns[i], textAlign: TextAlign.center, keyboardType: TextInputType.number, maxLength: 1, style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold), decoration: InputDecoration(counterText: '', border: OutlineInputBorder(borderRadius: BorderRadius.circular(10))), onChanged: (v) { if (v.isNotEmpty && i < 5) _fns[i+1].requestFocus(); if (i == 5 && v.isNotEmpty) _verify(); })))),
          const SizedBox(height: 32),
          ...
          TextButton(onPressed: _timer == 0 ? () => setState(() { _timer = 60; _countdown = Timer.periodic(const Duration(seconds: 1), (t) { if (_timer > 0) { setState(() => _timer--); } else { t.cancel(); } }); }) : null, child: Text(_timer > 0 ? 'إعادة الإرسال (${_timer}ث)' : 'إعادة الإرسال', style: TextStyle(color: _timer == 0 ? AppColors.primary : Colors.grey))),
        ]),
      ),
    );
  }
