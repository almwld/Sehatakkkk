...
class _VerificationScreenState extends State<VerificationScreen>{
  ...
  Future<void> _load() async{try{final u=FirebaseAuth.instance.currentUser;if(u==null)throw Exception('يجب تسجيل الدخول أولاً');final s=await FirebaseFirestore.instance.collection('users').doc(u.uid).get();final d=s.data()??{};if(!mounted)return;setState((){_role=d['role']?.toString()??'user';_name.text=d['name']?.toString()??d['displayName']?.toString()??'';_license.text=d['licenseNumber']?.toString()??'';_experience.text=d['experience']?.toString()??'';_specialty.text=d['specialty']?.toString()??'';_status=d['verificationStatus']?.toString()??'notSubmitted';_verified=d['isVerified']==true;_loading=false;});}catch(e){if(mounted)setState((){_error=e.toString();_loading=false;});}}
  ...
  @override Widget build(BuildContext context){if(_loading)return const Scaffold(body:Center(child:CircularProgressIndicator()));if(_error!=null)return Scaffold(body:Center(child:Text(_error!)));final dark=Theme.of(context).brightness==Brightness.dark;return Scaffold(backgroundColor:dark?const Color(0xFF0B1121):const Color(0xFFF8FAFC),appBar:CustomAppBar(title:'توثيق حساب '+_roleLabel,backgroundColor:AppColors.primary,foregroundColor:Colors.white,elevation:0),body:SingleChildScrollView(padding:const EdgeInsets.all(16),child:Column(children:[Card(child:Padding(padding:const EdgeInsets.all(16),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(_verified?'الحساب موثق':_status=='pending'?'الطلب قيد المراجعة':'يجب توثيق حسابك',style:const TextStyle(fontSize:20,fontWeight:FontWeight.bold)),const SizedBox(height:8),Text(_verified?'تم اعتماد حسابك ويمكنك استخدام ميزات دورك.':_status=='pending'?'تم إرسال ملفك إلى المشرف وستصلك النتيجة عبر الإشعارات.':'لاستخدام ميزات '+_roleLabel+' يجب استكمال التوثيق وإرفاق المؤهلات والمستندات المطلوبة.')]))),const SizedBox(height:12),_field(_name,'الاسم الكامل'),_field(_age,'العمر',type:TextInputType.number),_field(_specialty,'التخصص'),_field(_license,'رقم الترخيص/المزاولة'),_field(_experience,'سنوات الخبرة'),_field(_academic,'ملخص السجل الأكاديمي'),const SizedBox(height:6),..._specialRequirements.map((x)=>Align(alignment:AlignmentDirectional.centerStart,child:Padding(padding:const EdgeInsets.only(bottom:6),child:Text('• '+x)))),_upload('academicRecord','السجل الأكاديمي'),_upload('certificates','الشهادات والمؤهلات'),_upload('professionalRecord','السجل المهني/الخبرات'),_upload('healthRecord','السجل الصحي/اللياقة المهنية'),_upload('identity','الهوية والمستندات الرسمية'),const SizedBox(height:10),const Text('الملفات المدعومة: PDF و JPG و PNG — الحد الأقصى 12MB للملف.',style:TextStyle(color:Colors.grey,fontSize:12)),const SizedBox(height:16),SizedBox(width:double.infinity,child:ElevatedButton.icon(onPressed:(_submitting||_verified||_status=='pending')?null:_submitVerification,icon:Icon(_status=='pending'?Icons.hourglass_top_rounded:Icons.send_rounded),label:Text(_status=='pending'?'تم إرسال الطلب للمراجعة':_submitting?'جاري الإرسال...':'إرسال طلب التوثيق ورفع المستندات'),style:ElevatedButton.styleFrom(backgroundColor:AppColors.primary,foregroundColor:Colors.white,padding:const EdgeInsets.symmetric(vertical:14))))])));}
}
