...
class _VerificationScreenState extends State<VerificationScreen> {
  ...
  Future<void> _loadStatus() async {
    try {
      ...
      if (!mounted) return;
      setState(() { _status = data['verificationStatus']?.toString() ?? 'notSubmitted'; _verified = data['isVerified'] == true; _loading = false; });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _status = 'pending'; _verified = false; });
  ...
  String get _title {
    ...
    if (_status == 'pending') return 'طلب التوثيق قيد المراجعة';
    if (_status == 'rejected') return 'تم رفض طلب التوثيق';
    return 'توثيق حساب الطبيب';
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_loading) {
      ...
      body = Center(child: SingleChildScrollView(padding: const EdgeInsets.all(24), child: Column(children: [Icon(_verified ? Icons.verified : Icons.assignment_turned_in_outlined, size: 80, color: AppColors.primary), const SizedBox(height: 16), Text(_title, textAlign: TextAlign.center, style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black87)), const SizedBox(height: 10), Text(_description, textAlign: TextAlign.center, style: TextStyle(fontSize: 14, color: isDark ? Colors.white70 : Colors.grey.shade700)), const SizedBox(height: 28), if (!_verified && _status != 'pending') SizedBox(width: double.infinity, height: 50, child: ElevatedButton(onPressed: _submitting ? null : _submit, style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: Colors.white), child: _submitting ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Text('إرسال طلب التوثيق'))), const SizedBox(height: 12), SizedBox(width: double.infinity, height: 50, child: OutlinedButton(onPressed: () => Navigator.pop(context), child: const Text('العودة')))])));
    }
    return Scaffold(backgroundColor: isDark ? const Color(0xFF0B1121) : const Color(0xFFF8FAFC), appBar: CustomAppBar(title: 'توثيق الحساب', backgroundColor: AppColors.primary, foregroundColor: Colors.white, elevation: 0), body: body);
  }
}
