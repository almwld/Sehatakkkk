...
class _VisionTestScreenState extends State<VisionTestScreen> {
  ...
  final correct = const ['يمين', 'أسفل', 'يسار', 'أعلى', 'يمين', 'أسفل'];
  int _round = 0;
  int _score = 0;
  ...
  Widget _test() {
    ...
    return ListView(children: [
      const Card(child: Padding(padding: EdgeInsets.all(14), child: Text('اختبار تفاعلي أولي. اجلس على مسافة ثابتة ومريحة من الشاشة، لا تكبّر النص، واستخدم إضاءة جيدة. النتيجة لا تمثل قياس حدة بصر سريري.', style: TextStyle(height: 1.5)))),
  ...
  Widget _result() {
    ...
    return Center(child: ListView(shrinkWrap: true, children: [const Icon(Icons.visibility_rounded, size: 72, color: Color(0xFF3949AB)), const SizedBox(height: 14), const Text('تقييم الاختبار', textAlign: TextAlign.center, style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)), const SizedBox(height: 10), Text('$_score من ${correct.length}', textAlign: TextAlign.center, style: const TextStyle(fontSize: 36, fontWeight: FontWeight.bold)), const SizedBox(height: 10), Text(label, textAlign: TextAlign.center, style: const TextStyle(fontSize: 16)), const SizedBox(height: 18), const Card(child: Padding(padding: EdgeInsets.all(14), child: Text('هذه النتيجة للتوعية وليست وصفة نظارة أو تشخيصاً. عند تشوش مستمر أو ازدواجية أو ألم أو فقد مفاجئ للرؤية، اطلب تقييماً طبياً ولا تعتمد على الاختبار.'))), const SizedBox(height: 18), ElevatedButton(onPressed: _reset, child: const Text('إعادة الاختبار'))]));
  }
}