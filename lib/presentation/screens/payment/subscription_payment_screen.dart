...
class _SubscriptionPaymentScreenState extends State<SubscriptionPaymentScreen>{
  ...
  Widget build(BuildContext context) {
    ...
    final enough = (_balance ?? 0) >= widget.price;
    return Scaffold(
      backgroundColor: dark ? const Color(0xFF08131A) : const Color(0xFFF4F8F7),
      appBar: AppBar(title: const Text('مراجعة ودفع الاشتراك'), backgroundColor: dark ? const Color(0xFF08131A) : Colors.white, foregroundColor: dark ? Colors.white : Colors.black87, elevation: 0),
      body: ListView(padding: const EdgeInsets.fromLTRB(16, 12, 16, 30), children: [
        Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(gradient: const LinearGradient(colors: [AppColors.primary, AppColors.primaryDark]), borderRadius: BorderRadius.circular(24)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('تأكيد الباقة', style: TextStyle(color: Colors.white70)), const SizedBox(height: 5),
          Text(widget.planName, style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w900)), const SizedBox(height: 10),
          Text('${widget.price} ر.ي', style: const TextStyle(color: Colors.white, fontSize: 30, fontWeight: FontWeight.w900)),
          Text(widget.annual ? 'اشتراك سنوي' : 'اشتراك شهري', style: const TextStyle(color: Colors.white70)),
        ])),
        ...
        Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: dark ? const Color(0xFF122027) : Colors.white, borderRadius: BorderRadius.circular(20)), child: Column(children: [
          Row(children: [
            Container(width: 48, height: 48, decoration: BoxDecoration(color: AppColors.primary.withOpacity(.1), shape: BoxShape.circle), child: const Icon(Icons.account_balance_wallet_outlined, color: AppColors.primary)),
            const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('محفظة صحتك', style: TextStyle(fontWeight: FontWeight.w900)),
              Text(_loading ? 'جارٍ قراءة الرصيد...' : 'الرصيد المتاح: ${(_balance ?? 0).toStringAsFixed(2)} ر.ي', style: TextStyle(fontSize: 12, color: dark ? Colors.white70 : Colors.black54)),
            ])),
          ]),
          const SizedBox(height: 14), Divider(color: dark ? Colors.white12 : Colors.black12), const SizedBox(height: 12),
          _row('قيمة الاشتراك', '${widget.price} ر.ي'),
          _row('الرصيد بعد الخصم', _loading ? '—' : '${((_balance ?? 0) - widget.price).clamp(0, double.infinity).toStringAsFixed(2)} ر.ي'),
          if (!_loading && !enough) Column(children: [
            Padding(padding: const EdgeInsets.only(top: 10), child: Row(children: [
              const Icon(Icons.warning_amber_rounded, color: Colors.orange, size: 18), const SizedBox(width: 6),
              const Expanded(child: Text('الرصيد غير كافٍ. أضف رصيداً إلى المحفظة ثم أعد المحاولة.', style: TextStyle(color: Colors.orange, fontSize: 12))),
            ])),
            const SizedBox(height: 8),
            SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const WalletScreen())).then((_) => _loadBalance()), icon: const Icon(Icons.account_balance_wallet_outlined), label: const Text('فتح المحفظة وإضافة رصيد'))),
          ]),
        ])),
        const SizedBox(height: 16),
        Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: AppColors.primary.withOpacity(.07), borderRadius: BorderRadius.circular(16)), child: const Row(children: [Icon(Icons.lock_outline, color: AppColors.primary), SizedBox(width: 9), Expanded(child: Text('يتم الخصم الذري من المحفظة على الخادم، وتُنشأ معاملة وفاتورة إلكترونية مرتبطة بالاشتراك.'))])),
