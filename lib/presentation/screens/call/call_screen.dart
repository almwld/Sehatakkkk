...
class _CallScreenState extends State<CallScreen> {
  final _live=LiveKitService(), _calls=CallService(); Room? _room; StreamSubscription<CallModel?>? _callSub; Timer? _timer; String? _callId,_roomName,_error; VideoTrack? _remote,_local; int _seconds=0; DateTime? _connectedAt; bool _connecting=true,_muted=false,_camera=true,_speaker=false,_joined=false,_ending=false;
  @override void initState(){super.initState();_connect();}
  Future<void> _connect() async { try { final user=FirebaseAuth.instance.currentUser; if(user==null) throw StateError('يجب تسجيل الدخول'); CallModel? call;
      if(widget.isOutgoing){ call=await _calls.initiateCall(receiverId:widget.doctorId,receiverName:widget.doctorName,receiverPhotoUrl:widget.doctorImage,type:widget.isVideo?CallType.video:CallType.audio,chatId:widget.chatId,idempotencyKey:'call_${DateTime.now().microsecondsSinceEpoch}'); }
      else if(widget.callId!=null){ call=await _calls.streamCall(widget.callId!).first; }
      if(call==null) throw StateError('تعذر العثور على المكالمة'); _callId=call.id; _roomName=(call.liveKitRoomName?.trim().isNotEmpty==true?call.liveKitRoomName:'call_${call.id}');
      _callSub=_calls.streamCall(call.id).listen((c){ if(!mounted||c==null)return; if(c.status==CallStatus.connected&&!_joined){_join(c,user);} else if(c.status==CallStatus.cancelled||c.status==CallStatus.rejected||c.status==CallStatus.missed||c.status==CallStatus.ended){if(!_ending)_finishFromRemote();} });
      if(call.status==CallStatus.connected && !_joined) await _join(call,user);
      if(mounted)setState(()=>_connecting=!_joined);
    }catch(e){debugPrint('Call connect: $e');if(mounted)setState((){_connecting=false;_error=e.toString();});ToastService.showError('فشل تجهيز المكالمة');}}
  Future<void> _join(CallModel call, User user) async { if(_joined||_ending)return; if(widget.isVideo&&!(await Permission.camera.request()).isGranted)throw StateError('يرجى منح إذن الكاميرا'); if(!(await Permission.microphone.request()).isGranted)throw StateError('يرجى منح إذن الميكروفون');
    _room=await _live.startCall(roomName:_roomName!,callerName:user.displayName??widget.doctorName,isVideo:widget.isVideo); _joined=true; _connectedAt=call.connectedAt?.toDate()??DateTime.now(); _seconds=DateTime.now().difference(_connectedAt!).inSeconds.clamp(0,1<<30); _timer=Timer.periodic(const Duration(seconds:1),(_){if(mounted&&_connectedAt!=null)setState(()=>_seconds=DateTime.now().difference(_connectedAt!).inSeconds.clamp(0,1<<30));});
    _room!.events.on<ParticipantConnectedEvent>((e)=>_bind(e.participant)); _room!.events.on<TrackSubscribedEvent>((e)=>_bind(e.participant)); _room!.events.on<TrackPublishedEvent>((e)=>_bind(e.participant)); _room!.events.on<ParticipantDisconnectedEvent>((_) {if(mounted)setState(()=>_remote=null);}); for(final p in _room!.remoteParticipants.values){_bind(p);} final p=_room!.localParticipant;if(p!=null)_bind(p); if(mounted)setState(()=>_connecting=false);
  }
  ...
  Future<void> _finishFromRemote() async {if(_ending)return;_ending=true;_timer?.cancel();_callSub?.cancel();SoundManager().stopAll();await _live.endCall();if(mounted)Navigator.pop(context);}
  Future<void> _end() async {if(_ending)return;_ending=true;_timer?.cancel();_callSub?.cancel();SoundManager().stopAll();try{if(_callId!=null)await _calls.endCall(_callId!,durationSeconds:_joined?_seconds:0);}catch(e){debugPrint('call state end: $e');}await _live.endCall();if(mounted)Navigator.pop(context);}
  ...
  @override void dispose(){_timer?.cancel();_callSub?.cancel();SoundManager().stopAll();if(_joined)_live.endCall();super.dispose();}
