...
class _StepTrackerScreenState extends State<StepTrackerScreen> {
  ...
  Future<void> _initialize() async {
    try {
      final started = await _service.startStepTracking();
      await _reload();
      if (!mounted) return;
      setState(() {
        _isTracking = started;
        ...
        child: Column(
          children: [
            ...
            ClipRRect(
              ...
              child: LinearProgressIndicator(value: _progress, minHeight: 12),
            ),
            if (_progress >= 1)
              const Padding(
                padding: EdgeInsets.only(top: 10),
                child: Text('🎉 تم تحقيق الهدف اليومي', style: TextStyle(fontWeight: FontWeight.bold)),
              ),
          ],
        ),
      ),
  ...
  Widget _weeklyChart() {
    if (_weeklyData.isEmpty) {
      return const SizedBox(height: 180, child: Center(child: Text('لا توجد بيانات محفوظة بعد')));
    }
    final spots = <FlSpot>[];
    for (var i = 0; i < _weeklyData.length && i < 7; i++) {
      final value = (_weeklyData[i]['steps'] as num?)?.toDouble() ?? 0;
            ...
            bottomTitles: AxisTitles(
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (value, meta) {
                  ...
                  final date = _weeklyData[index]['date']?.toString();
                  if (date == null) return const SizedBox.shrink();
                  final parsed = DateTime.tryParse(date);
                  return Padding(
                    padding: const EdgeInsets.only(top: 6),
                    child: Text(parsed == null ? '' : DateFormat('E', 'ar').format(parsed), style: const TextStyle(fontSize: 10)),
                  );
                },
              ),
            ),
          ...
          children: [
            ...
            Card(
              child: Padding(
                ...
                child: Text(
                  _isTracking
                      ? 'التتبع نشط أثناء استخدام التطبيق. أبقِ الهاتف معك أثناء المشي للحصول على قراءة أفضل.'
                      : 'تعذر تشغيل مستشعر النشاط. تحقق من أذونات النشاط والحساسات ثم أعد المحاولة.',
                ),
              ),
            ),
            const SizedBox(height: 8),
            const Text('* السعرات تقديرية وتعتمد على معامل ثابت؛ لا تُستخدم كقياس طبي أو غذائي دقيق.', style: TextStyle(fontSize: 11)),
          ],
        ),
      ),
    );
