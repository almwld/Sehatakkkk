...
class _DeliveryCompanyScreenState extends State<DeliveryCompanyScreen> {
  ...
  @override Widget build(BuildContext context) => Scaffold(appBar: CustomAppBar(title: 'اختر شركة التوصيل', backgroundColor: AppColors.primary, foregroundColor: Colors.white), body: _loading ? const Center(child: CircularProgressIndicator()) : _companies.isEmpty ? const Center(child: Text('لا توجد شركات توصيل متاحة')) : Column(children: [Expanded(child: ListView.builder(padding: const EdgeInsets.all(16), itemCount: _companies.length, itemBuilder: (_, i) { final c = _companies[i]; final selected = c.id == _selected; return Card(child: ListTile(onTap: () => setState(() => _selected = c.id), leading: const Icon(Icons.delivery_dining), title: Text(c.name), subtitle: Text('${c.rating} ★ • ${_service.calculateDeliveryFee(c, widget.distance).toStringAsFixed(0)} ريال • ${_service.estimateDeliveryTime(c, widget.distance)} دقيقة'), trailing: Icon(selected ? Icons.check_circle : Icons.radio_button_unchecked, color: selected ? AppColors.primary : Colors.grey)); })), Padding(padding: const EdgeInsets.all(16), child: SizedBox(width: double.infinity, height: 50, child: ElevatedButton(onPressed: _selected == null ? null : () { final c = _companies.firstWhere((x) => x.id == _selected); widget.onSelect(c); Navigator.pop(context); }, child: const Text('تأكيد التوصيل'))))]));
}
