...
class _CommunityScreenState extends State<CommunityScreen>
  ...
  Future<bool> _isVerifiedDoctor() async {
    ...
    return data['role']?.toString() == 'doctor' && data['isVerified'] == true;
  }
  ...
  Future<void> _openCreatePost() async {
    ...
    final verified = await _isVerifiedDoctor();
    if (!mounted) return;
    if (!verified) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (docs.isEmpty) {
            return const Center(child: Text('لا توجد منشورات منشورة حالياً'));
          }
...
class _PostCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final image = post.images?.isNotEmpty == true
        ? post.images!.first
        : post.imageUrl;
    return Container(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${post.category ?? 'عام'} • ${post.timeAgo}',
                        style: TextStyle(
                          fontSize: 10,
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return ListTile(
                        leading: CircleAvatar(
                          child: Text(
                            (d['userName']?.toString() ?? 'م')
                                .characters
                                .first,
                          ),
                        ),
                        title: Text(
                          d['userName']?.toString() ?? 'مستخدم',
