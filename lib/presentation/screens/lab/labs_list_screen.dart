...
class _LabsListScreenState extends State<LabsListScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadLabs() async {
    ...
    try {
      ...
      final labs = snapshot.docs.map((doc) => <String,dynamic>{'id':doc.id,...doc.data()}).where((l) => _string(l['name']).isNotEmpty).toList();
      labs.sort((a,b) => _number(b['rating']).compareTo(_number(a['rating'])));
      if (mounted) setState(() { _labs=labs; _isLoading=false; });
    } catch (_) {
      if (mounted) setState(() { _isLoading=false; _error='تعذر تحميل بيانات المختبرات. تحقق من الاتصال ثم حاول مرة أخرى.'; });
    }
  }
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body:_isLoading?const Center(child:CircularProgressIndicator(color:AppColors.primary)):_error!=null?_errorView(dark):Column(children:[
  ...
  Widget _errorView(bool dark)=>Center(child:Padding(padding:const EdgeInsets.all(24),child:Column(mainAxisSize:MainAxisSize.min,children:[Icon(Icons.wifi_off_rounded,size:64,color:dark?Colors.grey[500]:Colors.grey[400]),const SizedBox(height:12),Text(_error!,textAlign:TextAlign.center,style:TextStyle(color:dark?Colors.grey[300]:Colors.grey[700])),const SizedBox(height:16),ElevatedButton.icon(onPressed:_loadLabs,icon:const Icon(Icons.refresh),label:const Text('إعادة المحاولة'))])));
  Widget _listView(List<Map<String,dynamic>> labs,bool dark){
    if(labs.isEmpty)return Center(child:Column(mainAxisSize:MainAxisSize.min,children:[Icon(Icons.science_outlined,size:64,color:dark?Colors.grey[600]:Colors.grey[400]),const SizedBox(height:12),Text(_searchQuery.isNotEmpty?'لا توجد نتائج مطابقة للبحث.':'لا توجد مختبرات مسجلة حاليًا.',style:TextStyle(color:dark?Colors.grey[300]:Colors.grey[700],fontSize:16)),const SizedBox(height:6),Text('ستظهر بيانات المختبرات هنا عند توفرها.',style:TextStyle(color:dark?Colors.grey[500]:Colors.grey[500]))]));
