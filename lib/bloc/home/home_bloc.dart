...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  HomeBloc({HomeRepositoryFixed? repository})
      : _repository = repository ?? HomeRepositoryFixed(),
  ...
  Future<void> _onStarted(HomeStarted event, Emitter<HomeState> emit) async {
    ...
    await _fetchAllData(emit, initialLoad: true);
  }
  ...
  void _onBannerChanged(
      HomeBannerChanged event, Emitter<HomeState> emit) {
  ...
  Future<void> _fetchAllData(
    ...
    emit(state.copyWith(
      status: initialLoad ? HomeStatus.loading : HomeStatus.refreshing,
      clearError: true,
      bannerImages: ImageKit.bannerList,
    ));

    final user = await _safe(
      () => _repository.getUserData(),
      (isLoggedIn: false, userName: 'مستخدم'),
    );
    final stats = await _safe(
      () => _repository.getHealthStats(),
      (calories: 0.0, steps: 0.0, sleep: 0.0, heartRate: 0.0),
    );
    final healthScore = await _safe(
    ...
    final notifications = await _safe(
