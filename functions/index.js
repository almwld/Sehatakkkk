...
exports.createPayment = onCall(async (request) => {
  ...
  const idempotencyKey = request.data.idempotencyKey
    ? String(request.data.idempotencyKey)
    : digest(`${uid}|${orderId || ''}|${amount}|${serviceId || ''}|${Date.now()}`);
...
exports.submitTopUp = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    tx.set(txRef, {
      ...
      walletName,
      idempotencyKey: key,
      metadata: request.data.metadata || null,
...
exports.requestRefund = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    tx.set(refundRef, {
      ...
      type: 'refund',
      status: 'pending',
      title: `طلب استرداد: ${data.title || 'عملية دفع'}`,
      description: `سبب الاسترداد: ${reason}`,
      orderId: data.orderId || null,
      serviceId: data.serviceId || null,
      serviceType: data.serviceType || null,
      metadata: {originalTransactionId: originalId, reason},
      createdAt: FieldValue.serverTimestamp(),
      completedAt: null,
    });
  });
...
exports.requestWithdrawal = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    tx.set(txRef, {
      ...
      type: 'withdrawal',
      status: 'pending',
      title: `طلب سحب عبر ${walletName}`,
      description: `وجهة السحب: ${destination}`,
      walletName,
      referenceNumber: destination,
      idempotencyKey: key,
      createdAt: FieldValue.serverTimestamp(),
      completedAt: null,
    });
...
exports.reviewTransaction = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (data.type === 'deposit') {
      tx.update(walletRef, {
        balance: Number(wallet.balance || 0) + amount,
        totalDeposited: Number(wallet.totalDeposited || 0) + amount,
        updatedAt: now,
        lastTransactionAt: now,
      });
      ...
      tx.update(walletRef, {
        balance: Number(wallet.balance || 0) + amount,
        totalSpent: Math.max(0, Number(wallet.totalSpent || 0) - amount),
        updatedAt: now,
        lastTransactionAt: now,
      });
      ...
      throw new HttpsError('failed-precondition', 'نوع المعاملة لا يدعم المراجعة');
    }
