...
function requireAuth(request) { if (!request.auth?.uid) throw new HttpsError('unauthenticated', 'يجب تسجيل الدخول أولاً'); return request.auth.uid; }
function amount(value, allowZero = false) { const n = Number(value); if (!Number.isFinite(n) || n < 0 || n > 100000000 || (!allowZero && n <= 0)) throw new HttpsError('invalid-argument', 'مبلغ غير صالح'); return Math.round(n * 100) / 100; }
function text(value, field, max = 300) { const v = String(value ?? '').trim(); if (!v || v.length > max) throw new HttpsError('invalid-argument', `الحقل ${field} غير صالح`); return v; }
function digest(v) { return crypto.createHash('sha256').update(v).digest('hex').slice(0, 40); }
const DELIVERY_FEE = 500;
// Demo catalog is intentionally retained for real UI/cart/wallet/invoice interaction.
// The client never supplies the authoritative price or stock state.
const pharmacyCatalog = Object.freeze({
  panadol_extra: {name: 'بانادول إكسترا', price: 1200, category: 'مسكنات', requiresPrescription: false, inStock: true},
  amoxicillin_500: {name: 'أموكسيسيلين 500mg', price: 3500, category: 'مضادات حيوية', requiresPrescription: true, inStock: true},
  vitamin_c: {name: 'فيتامين سي', price: 800, category: 'فيتامينات', requiresPrescription: false, inStock: true},
  omeprazole_20: {name: 'أوميبرازول 20mg', price: 2500, category: 'مضادات الحموضة', requiresPrescription: false, inStock: true},
  metronidazole: {name: 'مترونيدازول', price: 1800, category: 'مضادات حيوية', requiresPrescription: true, inStock: false},
  aloe_gel: {name: 'جل الصبار', price: 1500, category: 'عناية بالبشرة', requiresPrescription: false, inStock: true},
});
